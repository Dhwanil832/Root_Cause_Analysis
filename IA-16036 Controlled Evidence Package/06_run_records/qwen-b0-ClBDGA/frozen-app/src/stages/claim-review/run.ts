  ...
  for (const batch of batchesBySize(claims, 9_000, 4)) {
    ...
    const schema = z.object({
      reviews: z.array(z.object({
        claimId: z.enum(batch.map(c => c.id) as [string, ...string[]]),
    ...
    for (const page of pages.length ? pages : [[]]) {
      const result = await runAgentStage({ stage: 'claim-review', model, schema, schemaName: 'claim_review',
        packet: { candidates: batch, sources: page, sourcePages: Math.max(1, pages.length),
          instruction: 'Assess ONLY the supplied passages. Not supported on this page means unsupported, not false or absent everywhere.' },
        validateOutput: output => {
          const problems: string[] = [];
          if (new Set(output.reviews.map(r => r.claimId)).size !== batch.length) problems.push('Review each candidate exactly once.');
          for (const row of output.reviews) {
            for (const citation of row.citations) {
              const source = page.find(s => s.sourceId === citation.sourceId);
              if (!source || !normalize(citation.excerpt) || !normalize(source.excerpt).includes(normalize(citation.excerpt)))
                problems.push(`Claim ${row.claimId}: citations must be exact contiguous passages from a supplied source.`);
            }
            if (row.verdict === 'supported' && (!row.citations.length || Object.values(row.checks).some(v => !v)))
              problems.push(`Claim ${row.claimId}: full support needs exact citations and all five checks satisfied.`);
          }
          return problems;
        },
      });
      results.push(result.output); traces.push(result.trace);
    }
    for (const claim of batch) {
      const rows = results.flatMap(r => r.reviews.filter(row => row.claimId === claim.id));
      ...
      const citations: Citation[] = selected.flatMap(r => r.citations.map(c => ({ ...c, label: sources.find(s => s.sourceId === c.sourceId)?.label || c.sourceId })));
