...
import { runManagedOperation, type ContextRecord } from '../causal-analysis/managed-operation';
  ...
  for (const targets of batchesBySize([...board.nodes, ...board.edges], 9_000, 3)) {
    ...
    for (const page of pages.length ? pages : [[]]) {
      const records: ContextRecord[] = [
        ...page.map(source => ({ id: source.sourceId, kind: 'source-passage', value: source })),
        ...claims.map(claim => ({ id: claim.id, kind: 'reviewed-claim-not-independent-evidence', value: claim })),
        ...(Array.isArray(evidence.answers) ? evidence.answers : []).map((answer, index) => ({ id: `answer-${index}`, kind: 'answer', value: answer })),
        ...(Array.isArray(evidence.conflicts) ? evidence.conflicts : []).map((conflict, index) => ({ id: `conflict-${index}`, kind: 'conflict', value: conflict })),
      ];
      const operation = await runManagedOperation({ model, stage: 'causal-verification', schema,
        schemaName: 'target_verification', records, packet: {
        originalIncident: evidence.originalIncident, targets,
