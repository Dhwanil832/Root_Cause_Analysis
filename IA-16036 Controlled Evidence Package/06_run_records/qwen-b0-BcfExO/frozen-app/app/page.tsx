    ...
    void Promise.resolve(context.registerTool({
      ...
      async execute(input: unknown) {
        ...
        const response = await fetch('/api/incidents', {
          ...
          body: JSON.stringify({ description: value.description.trim(), modelIds: value.modelIds }),
        });
        const payload = await response.json() as { id?: string; error?: string };
        if (!response.ok || !payload.id) throw new Error(payload.error || 'Unable to create the investigation.');
        window.location.href = `/incident/${payload.id}`;
    ...
    <main className="landing-shell">
      <header className="site-header">
        <Link className="brand" href="/" aria-label="RCA Fieldwork home">
          ...
          <span>RCA Fieldwork <small className="brand-edition">Try 4</small></span>
        </Link>
