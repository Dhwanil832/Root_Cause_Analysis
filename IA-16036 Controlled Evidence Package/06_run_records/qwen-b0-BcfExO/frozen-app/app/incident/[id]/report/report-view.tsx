...
function markdownReport(incident: IncidentRecord, trackId: string, versionId: string) {
  ...
  const lines = [
    ...
    '## Evidence claims', '',
    ...analysis.evidenceClaims.map((claim) => `- **${claim.status} · ${claim.kind}:** ${claim.text} _(sources: ${claim.sourceIds.join(', ') || 'not established'})_`), '',
    '## Causal findings', '',
    ...analysis.causalBoard.nodes.map((node) => `- **${node.type} · ${node.status}:** ${node.label} — ${node.detail}`), '',
    '### Causal links', '',
...
export function ReportView({ incidentId, trackId, versionId }: { incidentId: string; trackId: string; versionId: string }) {
  ...
  const ready = ['root-causes-accepted', 'corrective-actions-proposed', 'closed'].includes(analysis.status);
  ...
  return <main className="report-shell">
    ...
    <article className="report-page">
      <header><span className={`report-state ${ready ? 'ready' : 'draft'}`}>{ready ? 'Human-reviewed report' : 'Investigation draft'}</span><p>RCA FIELDWORK · TRY 3</p><h1>{incident.title}</h1><dl><dt>Model track</dt><dd>{track.modelName}</dd><dt>Version</dt><dd>V{version.number}</dd><dt>Investigation status</dt><dd>{analysis.status.replaceAll('-', ' ')}</dd><dt>Generated</dt><dd>{new Date().toLocaleString()}</dd></dl></header>
      ...
      <section><h2>Causal findings</h2><table><thead><tr><th>Type</th><th>Finding</th><th>Status</th><th>Sources</th></tr></thead><tbody>{analysis.causalBoard.nodes.map((node) => <tr key={node.id}><td>{node.type.replaceAll('-', ' ')}</td><td><strong>{node.label}</strong><br />{node.detail}</td><td>{node.status}</td><td>{node.sourceIds.join(', ') || 'Required'}</td></tr>)}</tbody></table></section>
      <section><h2>Evidence ledger</h2><table><thead><tr><th>Kind</th><th>Claim</th><th>Status</th><th>Sources</th></tr></thead><tbody>{analysis.evidenceClaims.map((claim) => <tr key={claim.id}><td>{claim.kind.replaceAll('-', ' ')}</td><td>{claim.text}</td><td>{claim.status}</td><td>{claim.sourceIds.join(', ') || 'Required'}</td></tr>)}</tbody></table></section>
