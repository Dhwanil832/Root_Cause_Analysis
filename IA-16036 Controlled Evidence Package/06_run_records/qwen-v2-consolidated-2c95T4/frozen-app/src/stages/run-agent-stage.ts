...
export async function runAgentStage<T extends DecisionOutput>(options: {
  ...
  const systemPrompt = `${options.systemPromptOverride ?? promptFor(options.stage, options.specialist)}\n\n${options.instruction || ''}`;
  ...
  const result = await recoverStage(options.stage, {
    ...
    ...(['causal-analysis', 'causal-verification'].includes(options.stage)
      ? { contextPolicy: 'managed-no-truncation-v1' } : {}),
  }, () => runProviderStage({
    stage: options.stage,
    model: options.model,
    systemPrompt,
