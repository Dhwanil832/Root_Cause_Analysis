...
export const FOCAL_NODE_KEY = 'focal';

/** This is a new output protocol, never a repair or migration of old output. */
export function materializeCausalGraph(output: CausalAnalysisOutput) {
  return {
    focalKey: FOCAL_NODE_KEY,
    nodes: [
      { ...output.focalEvent, key: FOCAL_NODE_KEY, type: 'focal-event' as const },
      ...output.nodes,
    ],
    edges: output.edges,
  };
...
export function causalOutputProblems(output: CausalAnalysisOutput): string[] {
  return causalReferenceProblems(materializeCausalGraph(output));
}
