...
import { compactCausalPacket } from '@/src/knowledge/causal-packet';

export async function runCausalAnalysis(model: ModelDescriptor, packet: unknown) {
  const result = await runAgentStage({
    stage: 'causal-analysis', model, schema: causalAnalysisSchema, schemaName: 'causal_analysis', packet: compactCausalPacket(packet),
    validateOutput: causalOutputProblems,
  });
