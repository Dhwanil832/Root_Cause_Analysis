...
export async function runEvidenceAdjudication(
  ...
  if (segments.length > 32) {
    throw new Error(`Source adjudication received ${segments.length} segments; its current output contract supports 32. No segments were dropped. Explicit evidence batching is required for this input.`);
  }
