...
import { CHECKPOINT_CONTEXT_VERSION, PROMPT_VERSION } from '@/src/prompts/manifest';
...
async function runForCommit(trackId: string, targetVersion: number, request: Omit<AnalysisRequest, 'onCheckpoint'>, strict: boolean,
  ...
  const identity = await fingerprint({
    promptVersion: CHECKPOINT_CONTEXT_VERSION, request,
...
export async function resumeTrack(trackId: string) {
  ...
  const work = (async () => {
    ...
    for (const row of rows.results) {
      ...
      if (pending?.kind === 'pending-investigation') {
        // Reuse the exact saved answers (including delivery timestamps) so
        // successful upstream calls retain their checkpoint identities.
        // Failed story application leaves its round ready, not applying.
        if (!pending.storyRoundId) return addTrackAnswers(trackId, pending.incoming || pending.answers, latest.number, undefined, true);
        const claim = await database().prepare([
          "UPDATE story_rounds SET status='applying',error='',updated_at=?",
          "WHERE id=? AND status='ready' AND base_version=? AND scenario_id IN",
          '(SELECT id FROM story_scenarios WHERE track_id=?)',
        ].join(' ')).bind(new Date().toISOString(), pending.storyRoundId, latest.number, trackId).run();
        if (!claim.meta.changes) throw new Error('The saved story round is not ready to resume for this track/version.');
        try {
          return await addTrackAnswers(trackId, pending.incoming || pending.answers, latest.number, pending.storyRoundId, true);
        } catch (error) {
          await database().prepare("UPDATE story_rounds SET status='ready',error=?,updated_at=? WHERE id=? AND status='applying'")
            .bind(error instanceof Error ? error.message : String(error), new Date().toISOString(), pending.storyRoundId).run();
          throw error;
        }
      }
    }
