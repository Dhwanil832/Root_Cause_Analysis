...
export async function runInvestigationCycle(input: InvestigationCycleInput): Promise<AnalysisSnapshot> {
  ...
  const sourceEvidence = { originalIncident: packet.originalIncident, answers: packet.answers, evidenceSegments };
  ...
  if (keyToId.size) {
    try {
      const result = await runCausalVerification(
        input.model,
        { ...sourceEvidence, evidenceClaims, conflicts, sourceAssessments, adjudicationConflicts },
        causalBoard,
        keyToId,
      );
      causalBoard = result.board;
