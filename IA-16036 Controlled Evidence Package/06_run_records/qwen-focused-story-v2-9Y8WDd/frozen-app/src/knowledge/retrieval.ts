...
import { buildEvidenceSegments } from './evidence-segments';
...
export function retrieveForQuestion(
  ...
  for (const segment of buildEvidenceSegments(documents)) {
    const lowered = segment.excerpt.toLowerCase();
    const score = needles.reduce((total, term) => total + (lowered.includes(term) ? 1 : 0), 0);
    if (score > 0) sources.push({
  ...
  for (const answer of answers) {
    ...
    const score = needles.reduce((total, term) => total + (lowered.includes(term) ? 1 : 0), 0);
    if (score > 0) sources.push({ sourceId: answer.sourceId || `answer:${answer.questionId}`, label: `Answer to ${answer.questionId}`, scope: 'answer', excerpt: excerpt(answer.text, needles), score });
  }
