import { MAX_STAGE_PACKET_CHARACTERS } from '../../knowledge/packet-limits';

const CONTEXT_FIELDS = ['claims', 'sources', 'answers', 'conflicts'] as const;

/** Bound the complete serialized input, not just its source excerpts.
...
export function verificationPacketPartitions<T extends {
  ...
  if (JSON.stringify(packet).length <= MAX_STAGE_PACKET_CHARACTERS) return [packet];
  ...
  function split(part: T) {
    ...
    if (JSON.stringify(annotate(part, Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)).length <= MAX_STAGE_PACKET_CHARACTERS) {
      parts.push(part);
      return;
    }
    ...
    if (!field) throw new Error('Causal verification contains an indivisible context record or target larger than the packet budget. No evidence was truncated; the record needs explicit lossless segmentation.');
