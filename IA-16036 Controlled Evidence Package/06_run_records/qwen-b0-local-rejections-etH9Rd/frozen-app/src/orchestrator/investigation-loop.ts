...
import { runClaimReview } from '@/src/stages/claim-review/run';
import { readEvidenceContext } from '@/src/stages/evidence-reading/run';
...
export function evidencePacket(input: InvestigationCycleInput, documentIntelligence: AnalysisSnapshot['documentIntelligence']) {
  return {
    ...
    evidenceSegments: buildEvidenceSegments(input.documents || []),
    documentCoverage: 'All nonblank extracted document text is supplied in contiguous evidenceSegments. Empty or failed extraction is not evidence of absence. Cite sourceId values, not JSON property paths.',
    ...
    previousVersion: input.previous
      ? {
          ...
          ).map(question => ({ id: question.id, text: question.text, status: question.status })),
          specialistScopes: input.previous.specialistKnowledgeBases.map(kb => ({ tagId: kb.tagId, label: kb.label })),
          contextScope: 'Foundation-stage view only. Each specialist receives its own prior findings separately. Causal analysis receives the previous board and reviewed claim ledger separately.',
...
export async function runInvestigationCycle(input: InvestigationCycleInput): Promise<AnalysisSnapshot> {
  ...
  const evidenceSegments = packet.evidenceSegments;
  const reading = await readEvidenceContext(input.model, input.incident, evidenceSegments);
  trace.push(...reading.traces);
  const reasoningPacket = { ...packet, evidenceSegments: reading.segments, documentCoverage: reading.coverage };
  const sourceEvidence = { originalIncident: packet.originalIncident, answers: packet.answers,
    evidenceSegments: reading.segments, documentCoverage: reading.coverage };

  let understanding: Awaited<ReturnType<typeof runIncidentUnderstanding>> | null = null;
  try {
    understanding = await runIncidentUnderstanding(input.model, reasoningPacket, answers);
  ...
  if (understanding) {
    try {
      const result = await runIncidentStructuring(input.model, reasoningPacket, understanding.output);
  ...
  try {
    const result = await runSpecialists(
      input.model,
      { ...sourceEvidence, structuredIncident: structured, sourceAssessments, adjudicationConflicts },
      tags,
      answers,
  ...
  const answerFetches: AnalysisSnapshot['answerFetches'] = [];
  ...
  try {
    const result = await runEvidenceProcessing(
      ...
      evidenceSegments,
      sourceAssessments,
      [...(input.previous?.evidenceClaims || []), ...fetchedClaims],
    );
    processedClaims = result.claims;
    conflicts = [...new Map([...(input.previous?.conflicts || []), ...result.conflicts].map(c => [c.id, c])).values()];
    trace.push(result.trace);
    await checkpoint('evidence-processing', 'completed', { claims: processedClaims, conflicts });
  } catch (error) {
    await fail('evidence-processing', error);
  }
  const specialistClaims: EvidenceClaim[] = knowledgeBases.flatMap(kb => kb.findings.map(finding => ({
    id: finding.id, text: finding.statement, kind: finding.type === 'hypothesis' ? 'inference' : 'record',
    status: 'proposed', proposedStatus: finding.status, sourceIds: finding.sourceIds, routedTo: [kb.tagId],
  })));
  const candidateClaims = mergeClaims(
  ...
  let evidenceClaims: EvidenceClaim[] = candidateClaims.map(claim => ({ ...claim, proposedStatus: claim.status, status: 'proposed' }));
  try {
    const result = await runClaimReview(input.model, candidateClaims, [
      { sourceId: 'incident-description', label: 'Original incident account', excerpt: input.incident },
      ...answers.map(answer => ({ sourceId: answer.sourceId || `answer:${answer.questionId}`, label: `Answer to ${answer.questionId}`, excerpt: answer.text })),
      ...evidenceSegments,
    ]);
    evidenceClaims = result.claims;
    trace.push(...result.traces);
    for (const claim of evidenceClaims.filter(c => c.review?.verdict === 'contradicted')) {
      conflicts.push({ id: stableId('conflict', `claim-review:${claim.id}`), summary: claim.review!.reason,
        claimIds: [claim.id], status: 'open', resolutionNeeded: 'Resolve the cited claim/source disagreement before using this claim causally.' });
    }
    await checkpoint('claim-review', 'completed', { claims: evidenceClaims });
  } catch (error) { await fail('claim-review', error); }

  const reviewedById = new Map(evidenceClaims.map(claim => [claim.id, claim]));
  for (const fetched of answerFetches) {
    const claims = fetched.claimIds.map(id => reviewedById.get(id));
    if (claims.some(c => c?.status === 'contradicted')) fetched.status = 'conflicting';
    else if (fetched.status === 'answered' && (!claims.length || claims.some(c => c?.status !== 'supported'))) {
      fetched.status = 'partial';
      fetched.userRequest ||= 'The generated answer is not fully supported by the cited passages. Please supply or clarify the missing evidence.';
    }
  }
  questions = questions.map(question => {
    if (answeredIds.has(question.id)) return question;
    const fetched = answerFetches.find(f => f.questionId === question.id);
    if (!fetched) return question;
    return { ...question, status: fetched.status === 'answered' ? 'answered' : fetched.status === 'partial' ? 'partially-answered' : fetched.status === 'conflicting' ? 'contradicted' : 'awaiting-user' };
  });
  ...
  try {
    const result = await runCausalAnalysis(input.model, {
      ...sourceEvidence,
      structuredIncident: structured,
      specialistKnowledgeBases: knowledgeBases.map(kb => ({ tagId: kb.tagId, label: kb.label,
        findings: kb.findings, questionIds: kb.questionIds, handoffs: kb.handoffs })),
      evidenceClaims,
      conflicts,
      sourceAssessments,
  ...
  if (keyToId.size) {
    try {
      const result = await runCausalVerification(
        input.model,
        { ...sourceEvidence, evidenceSegments, evidenceClaims, conflicts, sourceAssessments, adjudicationConflicts },
        causalBoard,
        keyToId,
      );
      causalBoard = result.board;
      ...
      trace.push(result.trace);
      await checkpoint('causal-verification', 'completed', {
        board: causalBoard, questions: verificationQuestions,
        rawModelOutput: result.output, providerAttempts: result.providerAttempts,
      });
    } catch (error) {
