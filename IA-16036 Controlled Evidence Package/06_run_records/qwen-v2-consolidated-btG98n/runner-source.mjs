// Checkpoint restart: preserve upstream evidence and reuse ONLY old discovery.
// New consolidation, linking and verification cannot reuse old decisions.
// Read frozen V2, preserve every upstream output, and
...
const reuseDirectory=process.argv[2] ? path.resolve(process.argv[2]) : path.join(path.dirname(source),'qwen-v2-managed-causal-RPVgjT');
const allowedReuse=directory=>['qwen-v2-managed-causal-','qwen-v2-consolidated-'].some(prefix=>directory.startsWith(path.dirname(source)+path.sep+prefix));
if(!allowedReuse(reuseDirectory))throw Error('Invalid isolated replay checkpoint source');
const dir=await fs.mkdtemp(path.join(path.dirname(source),'qwen-v2-consolidated-'));
...
await save('manifest.json',{kind:'saved-discovery-consolidation-linking-verification-restart',createdAt:new Date().toISOString(),
...
await fs.mkdir(path.join(dir,'phases'));
await fs.copyFile(fileURLToPath(import.meta.url),path.join(dir,'runner-source.mjs'));
const phaseSink=async(phase,value)=>{
  await save(`phases/${phase}.json`,value);
  console.log(JSON.stringify({event:'phase-saved',phase,processed:value.processed,total:value.total,nodes:value.board?.nodes.length,edges:value.board?.edges.length,at:new Date().toISOString()}));
};
globalThis.fetch=async(url,options)=>{
  ...
  assert.ok(!body.format?.properties?.nodes,'Discovery must be reused, not rerun in this checkpoint restart');
...
const cache=new Map(),preservedCheckpoints=[];
if(reuseDirectory) {
  const visited=new Set();
  async function loadCache(directory) {
    if(visited.has(directory))throw Error('Checkpoint lineage cycle');
    if(!allowedReuse(directory))throw Error('Invalid checkpoint lineage');
    ...
    for(const name of (await fs.readdir(path.join(directory,'checkpoints'))).sort()) {
      ...
      if(checkpoint.kind==='model-call') {
        const discovery=Boolean(checkpoint.requestIdentity?.schema?.properties?.nodes);
        const fromNewRestart=path.basename(directory).startsWith('qwen-v2-consolidated-');
        if(discovery||fromNewRestart) {
          cache.set(checkpoint.key,checkpoint.response);
          preservedCheckpoints.push({file:path.join(directory,'checkpoints',name),sha256:sha(await fs.readFile(path.join(directory,'checkpoints',name))),key:checkpoint.key,discovery});
        }
      }
    }
  }
...
assert.equal(new Set(preservedCheckpoints.filter(c=>c.discovery).map(c=>c.key)).size,10,'All ten completed discovery operations must be available');
await save('preserved-checkpoints.json',{discoveryOperations:10,oldLinkingAndVerificationReused:false,checkpoints:preservedCheckpoints});
...
try {
  await withStageRecovery(recovery,async()=>{
    ...
    const causal=await runCausalAnalysis(model,packet,phaseSink);
    ...
    const verified=await runCausalVerification(model,packet,causal.board,causal.keyToId,phaseSink);
