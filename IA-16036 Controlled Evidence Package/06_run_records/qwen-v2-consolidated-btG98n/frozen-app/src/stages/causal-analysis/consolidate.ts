...
export function consolidationProblems(rows: Record<string, Disposition>, existing: Set<string>, candidates: Set<string>, _claimIds: Set<string>) {
  const errors: string[] = [];
...
export async function consolidateCandidates(model: ModelDescriptor, packet: unknown, drafts: DraftNode[],
  ...
  for (const page of batchesBySize(drafts, 7_000, 8)) {
    ...
    const rowProblems = consolidationProblems(rows, new Set(catalog.map(n => n.key)), new Set(page.map(d => d.key)), new Set(index.claims.keys()));
