...
function initialPositions(nodes: CausalNode[]) {
  ...
  return Object.fromEntries(nodes.map((node) => {
    ...
    return [node.id, { x, y: 28 + index * 280 }];
...
export function CausalCanvas({
  ...
  const height = Math.max(520, ...Object.values(positions).map((position) => position.y + 280));
  ...
  return (
    <div className="interactive-causal-wrap">
      <div className="canvas-toolbar">
        <div><strong>Investigation canvas · {provisional ? 'Pending V' : 'V'}{versionNumber}</strong><span>{provisional ? 'Saved checkpoint only — not a committed version or an approved RCA. Investigation and verification may be incomplete; review actions are disabled.' : 'Drag nodes to organize the working theory. Layout is a local viewing preference; review decisions create a new immutable version.'}</span></div>
        {!provisional && <label>Decision note<input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Optional rationale" /></label>}
        ...
        <div className="causal-stage" style={{ width: 1630, height, transform: `scale(${zoom})` }}>
          <svg className="causal-edge-layer" width="1630" height={height} aria-hidden="true">
            ...
            {board.edges.map((edge) => {
              ...
              return <g key={edge.id}><line x1={from.x + 215} y1={from.y + 74} x2={to.x} y2={to.y + 74} className={`edge-${unresolved ? 'unknown' : edge.status}`} markerEnd="url(#causal-arrow)" /><title>{unresolved ? 'UNRESOLVED' : 'EVIDENCE-REVIEWED'} · {edge.type}: {edge.rationale} {findingsFor(edge.id).map(f => f.issue).join(' ')}</title></g>;
            })}
          </svg>
          {board.nodes.map((node) => {
            ...
            >
              ...
              <small>Factual support: {node.status} · Role: {node.roleStatus || 'not assessed'}</small>
              {node.roleReason && <small>{node.roleReason}</small>}
              ...
              {!provisional && REVIEWABLE.has(node.type) && <div className="node-review-actions">
      ...
      <details className="edge-review-list"><summary>Connection evidence and unresolved findings ({board.edges.length})</summary>{board.edges.map(edge => <article key={edge.id}>
        ...
        {!!edge.jointConditions?.length&&<p>Joint requirements (all required, not alternative evidence): {edge.jointConditions.map(id=>board.nodes.find(n=>n.claimIds?.includes(id))?.label||id).join(' AND ')}</p>}
