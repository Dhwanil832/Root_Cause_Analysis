...
export function EngineProgress({ progress, onRetry, onRefresh, onPause }: {
  ...
  return <section className="result-section" aria-live="polite">
    <div className="section-heading"><div><p className="eyebrow">Try 7 evidence-led revision engine</p><h2>Execution: {progress.status}</h2></div></div>
    ...
    {progress.assessmentGate&&<div><strong>Assessment gate: {progress.assessmentGate.status}</strong><p>This is an execution check, not an RCA performance score or a guarantee of defect-free software.</p>{progress.assessmentGate.issues.map((issue,i)=><p key={i}>{issue}</p>)}</div>}
    ...
    {progress.status === 'partial' && failures.length>0 && <button className="button" type="button" onClick={onRetry}>Retry failed work in a new revision</button>}
    {progress.status === 'partial' && !failures.length && <p>No failed whole tasks to retry. Review the rejected items and unresolved questions; supply corrected or additional evidence for a new version.</p>}
    ...
    <details><summary>Tasks, evidence coverage, and failures</summary>
      {progress.tasks.map(t => <article className="question-card" key={t.id}>
        ...
        {t.failureCategory&&<p>Failure attribution: {t.failureCategory}. See the exact request and response before attributing RCA quality.</p>}
