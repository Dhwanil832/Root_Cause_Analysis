...
import type { IncidentRecord, EvidenceClaim } from '@/src/domain/types';
import { RejectedConnections } from '../../rejected-connections';

const claimStatus=(claim:EvidenceClaim)=>claim.reviewExecution&&claim.reviewExecution!=='complete'
  ? `Review ${claim.reviewExecution}` : claim.review?claim.status:'Not reviewed';
...
function markdownReport(incident: IncidentRecord, trackId: string, versionId: string) {
  ...
  const lines = [
    ...
    `**Execution:** ${version.executionStatus || (analysis.stageErrors.length ? 'Incomplete — stage failures recorded' : 'Legacy snapshot')}. Model execution is not proof of RCA correctness or human acceptance.`, '',
    '## Provisional narrative', '', 'This narrative is not proof that proposed revisions were accepted. Read the factual/role status, execution gate, and change records.', '', analysis.structuredIncident.summary, '',
    '## Original incident account', '', incident.description, '',
    '## Reported event', '', `**Focal event:** ${analysis.structuredIncident.focalEvent}`, '', `**Actual impact:** ${analysis.structuredIncident.actualImpact}`, '', `**Potential impact:** ${analysis.structuredIncident.potentialImpact}`, '',
    ...
    '## Evidence claims', '',
    ...analysis.evidenceClaims.map((claim) => `- **${claimStatus(claim)} · ${claim.kind}:** ${claim.text} _(sources: ${claim.sourceIds.join(', ') || 'not established'})_${claim.review ? `\n  Review: ${claim.review.reason}\n${claim.review.citations.map(c => `  Source ${c.sourceId}: “${c.excerpt}”`).join('\n')}` : ''}`), '',
    '## Causal findings', '',
    ...analysis.causalBoard.nodes.filter(n=>n.type!=='focal-event').map(n=>`- ${n.label}: factual support **${n.status}**; role **${n.roleStatus||'not-assessed'}**. ${n.roleReason||''}`), '',
    `Execution assessment gate: ${analysis.engineProgress?.assessmentGate?.status||'not-assessed'}. Not an external RCA score.`,
    ...(analysis.engineProgress?.assessmentGate?.issues||[]).map(issue=>`- ${issue}`),'',
    ...
    '### Rejected connection proposals — excluded from the active board', '',
    ...(analysis.causalBoard.rejectedEdges || []).map(edge => `- **Rejected: ${edge.fromLabel} → ${edge.toLabel}**\n  Reasons: ${edge.reasons.join(' ')}\n  Original proposed status: ${edge.proposal.status}; relationship: ${edge.proposal.type}\n  Original rationale: ${edge.proposal.rationale}\n  Counterfactual: ${edge.proposal.counterfactual}\n  Competing explanation: ${edge.proposal.competingExplanation}\n  Evidence gap: ${edge.proposal.evidenceGap}\n  Sources: ${edge.proposal.sourceIds.join(', ')}; claims: ${edge.proposal.claimIds.join(', ')}`), '',
    ...
    ...(analysis.conflicts.length ? analysis.conflicts.map((conflict) => `- **Source conflict ${conflict.status}:** ${conflict.summary} — ${conflict.resolutionNeeded}\n${(conflict.sides||[]).map((side,i)=>`  Account ${i+1}: ${side.assertion}\n${side.originalQuotes.map(q=>`    ${q.source}, line ${q.line}: “${q.quote}”`).join('\n')}`).join('\n')}`) : ['No source conflicts are recorded in this version.']), '',
...
export function ReportView({ incidentId, trackId, versionId }: { incidentId: string; trackId: string; versionId: string }) {
  ...
  return <main className="report-shell">
    <nav className="report-toolbar"><Link href={`/incident/${incident.id}`}>← Investigation</Link><div><button type="button" onClick={() => download(`${fileStem}.md`, 'text/markdown', report)}>Download Markdown</button><button type="button" onClick={() => download(`${fileStem}.json`, 'application/json', JSON.stringify({ incident: {id:incident.id,title:incident.title,description:incident.description}, track: {id:track.id,modelId:track.modelId,modelName:track.modelName,documents:track.documents.filter(d=>analysis.documentIds.includes(d.id))}, version }, null, 2))}>Evidence package</button><button type="button" onClick={() => window.print()}>Print / Save PDF</button></div></nav>
    <article className="report-page">
      <header><span className={`report-state ${ready ? 'ready' : 'draft'}`}>{ready ? 'Human decisions recorded — see open findings' : 'Investigation draft'}</span><p>RCA FIELDWORK · TRY 7</p><h1>{incident.title}</h1><dl><dt>Model track</dt><dd>{track.modelName}</dd><dt>Version</dt><dd>V{version.number}</dd><dt>Investigation status</dt><dd>{analysis.status.replaceAll('-', ' ')}</dd><dt>Execution</dt><dd>{version.executionStatus || (analysis.stageErrors.length ? 'Incomplete' : 'Legacy snapshot')} — not proof of RCA correctness</dd><dt>Generated</dt><dd>{new Date().toLocaleString()}</dd></dl></header>
      <section><h2>Provisional narrative</h2><p>This narrative is not proof that proposed revisions were accepted. Read the factual/role status, execution gate, and change records.</p><p>{analysis.structuredIncident.summary}</p></section>
      <section><h2>Execution assessment</h2><p>{analysis.engineProgress?.assessmentGate?.status||'Not assessed'} — not an external RCA score.</p>{analysis.engineProgress?.assessmentGate?.issues.map((issue,i)=><p key={i}>{issue}</p>)}</section>
      ...
      <section><h2>Causal findings</h2><table><thead><tr><th>Type</th><th>Finding</th><th>Evidence and role review</th><th>Sources</th></tr></thead><tbody>{analysis.causalBoard.nodes.map((node) => <tr key={node.id}><td>{node.type.replaceAll('-', ' ')}</td><td><strong>{node.label}</strong><br />{node.detail}</td><td>{reviewState(node.id, node.verified, node.status)}<br />Factual support: {node.status}<br/>Role: {node.roleStatus||'not-assessed'}<br/>{node.roleReason}</td><td>{node.sourceIds.join(', ') || 'Required'}</td></tr>)}</tbody></table></section>
      <section><h2>Evidence ledger</h2><table><thead><tr><th>Kind</th><th>Claim and review</th><th>Status</th><th>Sources</th></tr></thead><tbody>{analysis.evidenceClaims.map((claim) => <tr key={claim.id}><td>{claim.kind.replaceAll('-', ' ')}</td><td>{claim.text}{claim.review && <><p>{claim.review.reason}</p>{claim.review.citations.map((c, i) => <blockquote key={i}>{c.excerpt} — {c.sourceId}</blockquote>)}</>}</td><td>{claimStatus(claim)}</td><td>{claim.sourceIds.join(', ') || 'Required'}</td></tr>)}</tbody></table></section>
      ...
      <RejectedConnections edges={analysis.causalBoard.rejectedEdges} />
