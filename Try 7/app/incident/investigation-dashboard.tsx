...
import type { CausalBoard, IncidentRecord, InvestigationQuestion, TagId, TrackRecord, VersionRecord } from '@/src/domain/types';
import { TAG_LABELS } from '@/src/domain/tag-catalog';
import { CausalCanvas } from './causal-canvas';
import { EngineProgress } from './engine-progress';
import { InvestigationPosition } from './investigation-position';
...
function QuestionCard({
  ...
  async function save() {
    ...
    try {
      ...
      // The original upload is a separate source, not a truncated duplicate in testimony.
  ...
  return (
    <article id={`question-${question.id}`} className={'question-card' + (question.status === 'answered' ? ' answered' : '')}>
      <div className="question-topline">
        <span>{question.priority} priority</span>
        <span>{question.intent.replaceAll('-', ' ')}</span>
      </div>
      <h3>{question.text}</h3>
      <p>Status: {question.status} · {question.searchStatus||'not-yet-searched'}</p>
      {!!question.parts?.length&&<details><summary>Answer coverage by information need</summary>{question.parts.map(part=>{
        const coverage=question.coverage?.find(c=>c.partId===part.id);
        return <p key={part.id}><strong>{part.text}</strong> — {coverage?.status||'not-yet-searched'}<br/>{coverage?.answer} {coverage?.gap}</p>;
      })}</details>}
      {!!question.subscriptions?.length&&<details><summary>Who needs this answer and why</summary>{question.subscriptions.map((s,i)=><p key={i}><strong>{tagLabel(s.owner)}</strong>: {s.decision}<br/>If established: {s.ifPresent||'Not specified'}<br/>If ruled out: {s.ifAbsent||'Not specified'}</p>)}</details>}
      {!!question.deliveries?.length&&<details><summary>Answer delivery receipts</summary>{question.deliveries.map((d,i)=><p key={i}>{tagLabel(d.owner)} · answer revision {d.answerRevision} · {d.status}<br/>{d.implication}</p>)}</details>}
...
export function InvestigationDashboard({ incidentId }: { incidentId: string }) {
  ...
  useEffect(() => {
    if (!incident || (!resuming && incident.tracks.every(t => t.versions.length && !t.versions.some(v => ['queued','running'].includes(v.executionStatus || 'completed'))))) return;
  ...
  async function reviewCorrectiveAction(actionId: string) {
    ...
    const response = await fetch(`/api/tracks/${track.id}/review`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ targetType: 'corrective-action', targetId: actionId, action: 'approve',expectedVersion:version?.number }),
    });
  ...
  const savedBoard = [...pending].reverse().find(c =>
    c.status === 'completed' && ['causal-analysis', 'causal-verification'].includes(c.stage)
    && Boolean((c.payload as { board?: CausalBoard })?.board));
  const boardPreview = savedBoard && <section className="result-section">
    <h3>Saved, uncommitted board</h3>
    <p>{tagLabel(savedBoard.stage)} checkpoint · {formatTime(savedBoard.createdAt)}. This snapshot may predate the latest retry; it is not a completed investigation.</p>
    <CausalCanvas key={savedBoard.id} board={(savedBoard.payload as { board: CausalBoard }).board}
      trackId={track.id} versionNumber={savedBoard.targetVersion} decisions={[]} provisional
      onReviewed={async () => {}} />
  </section>;
  const progress = version?.analysis.engineProgress ? <EngineProgress progress={version.analysis.engineProgress} onRetry={resume} onRefresh={() => { void load(); }} onPause={()=>{void (async()=>{
    try {const response=await fetch(`/api/tracks/${trackId}/pause`,{method:'POST'});const result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||'Pause failed.');await load();}
    catch(error){setResumeError(error instanceof Error?error.message:'Pause failed.');}
  ...
  if (!version) return <main className="center-page"><div className="processing-card" style={savedBoard ? { width: '100%', maxWidth: 1600 } : undefined}>
    ...
    {progress}{boardPreview}<Link href="/history">Return to history</Link>
  </div></main>;
  ...
  const visibleDocuments = track.documents.filter((document) =>
    analysis.documentIds.includes(document.id),
  );
    ...
    <main className="dashboard-shell">
      ...
      <div className="dashboard-grid">
        ...
        <section className="investigation-main">
          ...
          <div className="version-strip" aria-label="Versions">
            {track.versions.map((item) => (
              <button type="button" key={item.id}
                ...
                <small>{item.executionStatus||'saved'}</small>
          ...
          {(analysis.engineProgress || pending.length > 0) && <>{progress}{resumeError&&<p role="alert" className="form-error">{resumeError}</p>}{boardPreview}</>}
          {analysis.investigationPosition && <InvestigationPosition position={analysis.investigationPosition} />}
          {analysis.investigationPosition && <section className="result-section causal-section">
            <div className="result-heading"><div><p className="eyebrow">Current causal map</p><h2>A working map with visible uncertainty</h2></div></div>
            <CausalCanvas board={analysis.causalBoard} trackId={track.id} versionNumber={version.number}
              key={version.id} provisional={['queued','running','paused'].includes(version.executionStatus||'')}
              decisions={analysis.humanDecisions || []} onReviewed={async () => { await load(true); }} />
          </section>}
          <p className="execution-state">Execution: {version.executionStatus || analysis.engineProgress?.status || (analysis.stageErrors.length ? 'incomplete — stage failures recorded' : 'legacy snapshot')} · Evidence review: {['queued','running','paused'].includes(version.executionStatus || analysis.engineProgress?.status || '') ? 'provisional — investigation and verification still in progress' : analysis.causalBoard.verificationFindings.some(f => f.severity === 'blocking') ? 'unresolved findings remain' : 'ready for human review; not human-approved'}</p>

          <div className="report-shortcuts">
            <a className="button ghost-button" href={`/api/tracks/${track.id}/export`}>Export experiment and traces</a>
            <a className="button ghost-button" href={`/api/tracks/${track.id}/export?format=zip`}>Archive with original files</a>
          ...
          <section className="result-section case-foundation">
            ...
            <div className="foundation-grid">
              ...
              <article><span>Model-described normal state — not independently verified</span><p>{analysis.structuredIncident.normalState}</p></article>
          ...
          <section className="result-section">
            <div className="result-heading">
              ...
              <span>{analysis.sourceAssessments.length?'Recorded source assessments are shown below.':'No separate source-authority adjudication is recorded. Original citations alone do not establish source reliability.'}</span>
            </div>
                ...
                <article className="document-intelligence-card" key={record.documentId}>
                  ...
                  {record.observations.length > 0 && <div className="observation-list">{record.observations.map((observation) => (
                    <article key={observation.id}>
                      ...
                      <small>{observation.location || 'Location not isolated'} · {analysis.engineProgress?'Confidence not calibrated':`${Math.round(observation.confidence * 100)}% model confidence`}{observation.requiresVerification ? ' · verification requested' : ''}</small>
            ...
            <div className="tag-grid">
              {analysis.tags.map((tag) => (
                <article className="tag-card" key={tag.id}>
                  <span className="confidence">{analysis.engineProgress?'Selected':`${Math.round(tag.confidence * 100)}%`}</span>
                  <h3>{tag.label}</h3>
                  <p>{tag.rationale}</p>
            ...
            {Object.entries(grouped).map(([tagId, questions]) => (
              <div className="question-group" key={tagId}>
                <div className="question-group-heading">
                  <span>{tagId === 'baseline' ? 'Baseline context' : tagLabel(tagId)}</span>
          ...
          <section className="result-section">
            <div className="result-heading">
              <div><p className="eyebrow">Evidence ledger</p><h2>{analysis.evidenceClaims.length} claims · {analysis.conflicts.length} source conflicts</h2></div>
              <span>Observation, record, testimony, interpretation, and assumption remain distinguishable.</span>
            </div>
            {analysis.conflicts.length > 0 && <div className="conflict-list">{analysis.conflicts.map((conflict) => <article key={conflict.id}><strong>{conflict.summary}</strong><p>{conflict.resolutionNeeded}</p><span>Source conflict: {conflict.status}</span>
              {conflict.scope&&<p>Scope: {[conflict.scope.subject,conflict.scope.location,conflict.scope.time].filter(Boolean).join(' · ')}</p>}
              {conflict.sides?.map((side,i)=><details key={i}><summary>Account {i+1}: {side.assertion}</summary>
                {side.originalQuotes.map((q,j)=><blockquote key={j}><p>{q.quote}</p><small title={q.source}>{q.label||q.source}, passage line {q.line}</small></blockquote>)}
                {side.citations.map((c,j)=><small key={j}>{c.label} · {c.sourceId}</small>)}
              </details>)}
            </article>)}</div>}
            <div className="claim-grid">{analysis.evidenceClaims.map((claim) => <article key={claim.id}><div><span>{claim.kind.replaceAll('-', ' ')}</span><b className={`evidence-status ${claim.status}`}>{claim.reviewExecution&&claim.reviewExecution!=='complete'?`review ${claim.reviewExecution}`:claim.review ? claim.status : 'not reviewed'}</b></div><p>{claim.text}</p><small>{claim.sourceIds.length ? `Sources: ${claim.sourceIds.join(', ')}` : 'Source still required'}</small>
              {claim.interpretation&&<details><summary>Fixed meaning · interpreted without evidence</summary>{claim.interpretation.parts.map(part=><section key={part.partId}>
                <p>{part.partId} · {part.claimType}: {part.assertion}</p><p>Must establish: {part.truthConditions}</p>
                <small>Scope: {[part.scope.subject,part.scope.location,part.scope.time].filter(Boolean).join(' · ')}</small>
                {part.ambiguities.map((a,i)=><p key={i}>Wording ambiguity: {a}</p>)}
              </section>)}</details>}
              {!!claim.scopeHistory?.length&&<details><summary>Claim scope history</summary>
                <p>Scope describes the assertion, not its truth or its eligibility for investigation.</p>
                {claim.scopeHistory.map((s,i)=><p key={i}>Revision {s.revision}: {s.claimType.replaceAll('-',' ')}</p>)}
              </details>}
              {claim.review && <details><summary>Evidence review: {claim.reviewExecution&&claim.reviewExecution!=='complete'?claim.reviewExecution:claim.review.claimParts?.status || claim.review.verdict}</summary><p>{claim.review.reason}</p>
                {claim.review.claimParts && <section aria-label="Claim-by-claim evidence review">
                  <p>Policy: {claim.review.claimParts.policy} · {claim.review.claimParts.policy.startsWith('literal-claim-')?'The complete original claim is reviewed without rewriting or splitting it.':'Final status calculated from the parts below.'}</p>
                  {claim.review.claimParts.issues.map((issue,index)=><p key={index}>{issue}</p>)}
                  {claim.review.claimParts.uncoveredText.length>0 && <p>Unreviewed wording: {claim.review.claimParts.uncoveredText.join(' ')}</p>}
                  {claim.review.claimParts.parts.map((part,index)=><details key={index}>
                    <summary>{claim.review!.claimParts!.policy.startsWith('literal-claim-')?'Complete claim':`Part ${index+1}`}: {part.status} — {part.claimText}</summary>
                    <p>Claim type: {part.claimType} · Model judgment: {part.relation}</p>
                    {part.truthConditions&&<p>Fixed truth conditions: {part.truthConditions}</p>}
                    <p>Evidence establishes: {part.establishes}</p><p>{part.reason}</p>
                    {part.counterexample?.possible && <p>Consistent counterexample: {part.counterexample.scenario}</p>}
                    {part.missingPremises.map((premise,i)=><p key={i}>Unproved premise: {premise}</p>)}
                    {part.issues.map((issue,i)=><p key={i}>Validation: {issue}</p>)}
                    {part.originalQuotes.map((quote,i)=><blockquote key={i}>{quote.claimText&&<p>Addresses claim wording: {quote.claimText}</p>}<p>{quote.quote}</p><small title={quote.source}>Original text: {quote.label||quote.source}, passage line {quote.line}. Copied by the application, not generated by the model.</small></blockquote>)}
                    {part.citations.map((citation,i)=><blockquote key={i}><p>{citation.excerpt}</p><small>Verified source passage: {citation.label} · {citation.sourceId}</small></blockquote>)}
                  </details>)}
                </section>}
                {!claim.review.claimParts && claim.review.citations.map((citation, index) => <blockquote key={index}><p>{citation.excerpt}</p><small>{citation.label} · {citation.sourceId}</small></blockquote>)}
              </details>}
            </article>)}</div>
          </section>
          ...
          <section className="result-section causal-section">
            ...
            {!analysis.investigationPosition && <CausalCanvas board={analysis.causalBoard} trackId={track.id} versionNumber={version.number}
              key={version.id} provisional={['queued','running','paused'].includes(version.executionStatus||'')}
              decisions={analysis.humanDecisions || []} onReviewed={async () => { await load(true); }} />}
            ...
            {!!analysis.causalBoard.branchRevisions?.length && <details className="verification-list"><summary>Changes to previous branches ({analysis.causalBoard.branchRevisions.length})</summary>
              <p>Model revision decisions, not independent evidence or human approval. Unresolved branches remain open for reconciliation.</p>
              {analysis.causalBoard.branchRevisions.map(change => <article key={change.previousTargetId}><span>{change.action}</span><strong>{change.previousLabel}</strong><p>{change.reason}</p><small>Evidence: {change.evidenceIds.join(', ') || 'No evidence cited'} · Current targets: {change.replacementIds.join(', ') || 'Unmapped'}</small></article>)}
            </details>}
              ...
              <div className="audit-content">
                {analysis.trace.map((entry) => (
                  <article key={entry.id}>
                    <span>{entry.stage} · {analysis.engineProgress?'Confidence not calibrated':`${Math.round(entry.confidence * 100)}% confidence`}</span>
                    <h3>{entry.summary}</h3>
