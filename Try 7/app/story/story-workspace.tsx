...
interface Workspace {
  ...
  rounds:{id:string;base_version:number;status:string;result_version:number|null;error:string;questions:{id:string;text:string}[];output:{output?:StoryOutput;releasedOutput?:StoryOutput;withheldQuestionIds?:string[];releaseDocuments?:{sourceId:string;questionId:string}[]}}[];
}
export function StoryWorkspace({trackId}:{trackId:string}) {
  ...
  useEffect(()=>{const timer=setTimeout(()=>void refresh().catch(e=>setError(String(e))),0);return()=>clearTimeout(timer);},[refresh]);
  ...
  return <main className="story-workspace">
    <header><p className="story-eyebrow">TRY 6 · EXTERNAL BENCHMARK SIMULATOR</p><h1>One model. Two separate roles.</h1><p>The story role holds a fixed scenario. The RCA role receives only the answers and records you release—not the hidden scenario or its private memory.</p>
    ...
    {!data?<p>Loading simulator…</p>:<>
      ...
      <section><h2>2. Generate answers</h2><p>Select a controlled subset or all pending questions. Each answer is a separate durable task with relevant source passages and released memory. No RCA causal board or evaluator answer key is sent.</p>
        <button onClick={()=>setSelected(data.questions.map(q=>q.id))}>Select all</button> <button onClick={()=>setSelected([])}>Clear selection</button>
        <details open><summary>{selected.length} of {data.questions.length} questions selected</summary><ol>{data.questions.map(q=><li key={q.id}><label><input type="checkbox" checked={selected.includes(q.id)} onChange={e=>setSelected(ids=>e.target.checked?[...ids,q.id]:ids.filter(id=>id!==q.id))}/>{q.text}</label></li>)}</ol></details>
        <button disabled={!data.scenario||active||(!selected.length&&!current)||!!current&&!['failed','partial','paused'].includes(current.status)} onClick={()=>void action('generate',{questionIds:selected})}>{current?.status==='paused'?'Resume saved story work':current&&['failed','partial'].includes(current.status)?'Retry only failed story tasks':'Queue selected answers'}</button>
        {active&&<p role="status">Working locally. This can take several minutes; completed rounds are preserved here.</p>}
      </section>
      <section><h2>3. Review and release</h2><p>Review source support before release. Applying a batch queues a separate RCA revision; it does not rerun the storyteller. Failed questions remain visible and are never fabricated as answers. Unknown and partial responses are not automatically resolved.</p>
        ...
        {[...data.rounds].reverse().map(round=><article key={round.id} className="story-round"><h3>From V{round.base_version} · {round.status}{round.result_version?` → V${round.result_version}`:''}</h3>
          ...
          {round.output.releasedOutput&&<p>{round.output.releasedOutput.answers.length} answer(s) released · {round.output.withheldQuestionIds?.length||0} withheld after review · {round.output.releaseDocuments?.length||0} original record(s) attached.</p>}
          {round.output.output?.answers.map(a=><details key={a.questionId}><summary>{round.questions.find(q=>q.id===a.questionId)?.text||a.questionId} <small>— model status: {a.status}{round.output.withheldQuestionIds?.includes(a.questionId)?' · WITHHELD — not sent to RCA':''}</small></summary><p>{a.answer}</p>{a.citations.map((c,i)=><blockquote key={i}>{c.quote}<footer>Source: {c.sourceId}</footer></blockquote>)}<p>Limit: {a.limitation||'Synthetic benchmark evidence.'}</p></details>)}
          {['ready','partial'].includes(round.status)&&<button disabled={active||round.base_version!==data.version} onClick={()=>{if(window.confirm('Release the successfully generated synthetic answers and queue the next RCA version? Failed answers will not be released.'))void action('apply',{roundId:round.id});}}>Release saved answers → create V{round.base_version+1}</button>}
        </article>)}
      </section>
    </>}
  </main>;
