...
export function applyBoardTask(state: EngineState, task: EngineTask, raw: unknown, refs: References) {
  ...
  if (task.kind === 'causal') {
    ...
    target.status = out.status === 'supported' && !supporting.length ? 'unknown' : out.status;
    ...
    if ('from' in target && target.status==='supported' && [target.from,target.to].some(id=>id!==focalId
      && (state.propositions.find(p=>p.id===id)?.status!=='supported'||state.propositions.find(p=>p.id===id)?.roleAssessment?.status!=='valid'))) {
      target.status='unknown';target.reviewReason+=' One or more endpoints lack factual support or validated causal role.';
    }
