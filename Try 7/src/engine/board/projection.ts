...
import {assessmentGate} from '../validation/execution';

function status(s: EpistemicStatus): EvidenceStatus { return s === 'partial' ? 'partially-supported' : s; }
...
export function project(state: EngineState, input: EngineInput, execution: RunStatus): AnalysisSnapshot {
  ...
  const questions: InvestigationQuestion[] = state.questions.filter(q => !q.coveredBy && activeQuestion(state,q)).map(q => ({
    ...
    parts:q.parts,coverage:q.coverage,subscriptions:q.subscriptions,deliveries:q.deliveries,searchStatus:q.inventory?'searched':'not-yet-searched',
    citations: citations(q.spanIds), priority: 'medium', status: q.status === 'partial' ? 'partially-answered' : q.status,
  }));
  const activePropositions = state.propositions.filter(p => !state.investigation || state.investigation.selectedFindingIds.includes(p.findingId));
  const nodes = activePropositions.map(p => ({ id: p.id, type: p.type, label: p.label, detail: p.detail, status: status(p.status),
    sourceIds: sourceIds(p.spanIds), specialistIds: state.findings.find(f => f.id === p.findingId)?.tags || [],
    roleStatus:p.roleAssessment?.status||'not-assessed' as const,roleReason:p.roleAssessment?.reason||'Causal role has not been assessed.',
    verified: p.status === 'supported'&&p.roleAssessment?.status==='valid', claimIds: [p.findingId], assessmentKind: state.findings.find(f => f.id === p.findingId)?.kind === 'hypothesis' ? 'hypothesis' as const : 'observation' as const,
    humanStatus: p.humanStatus }));
  const ids = new Set(['focal', ...nodes.map(n => n.id)]);
  const edges = state.relationships.filter(e => ids.has(e.from) && ids.has(e.to) && (!state.investigation || e.proposedBy?.length)).map(e => ({ id: e.id, from: e.from, to: e.to, type: e.type,
    rationale: e.rationale, status: status(e.status), sourceIds: sourceIds(e.spanIds), claimIds: e.findingIds,
    counterfactual: e.counterfactual, competingExplanation: e.alternative, evidenceGap: e.gap,jointConditions:e.jointConditions, verified: e.status === 'supported' }));
  const findings = [...activePropositions, ...state.relationships.filter(e => !state.investigation || e.proposedBy?.length)].filter(p => p.status !== 'supported').map(p => ({
  ...
  return {
    investigationPosition: state.investigation ? {
      stage: state.investigation.stage, ready: state.investigation.ready, summary: state.investigation.summary, revisionReason: state.investigation.revisionReason,
      impacts:state.investigation.impacts,pendingImpacts:state.investigation.pendingImpacts,
      proposedNarrative:state.investigation.proposedNarrative,
      notebookCount: state.findings.length, selectedCount: state.investigation.selectedFindingIds.length,
      reusedSourceSpans: state.investigation.readSpanIds.filter(id => !state.tasks.some(t => t.kind === 'read' && t.targetIds.includes(id))).length,
      branches: state.investigation.branches.map(b => ({ ...b,
        supporting: b.supporting.map(id => state.findings.find(f => f.id === id)?.statement || id),
        opposing: b.opposing.map(id => state.findings.find(f => f.id === id)?.statement || id) })),
      directions: state.investigation.directions.filter(d => !state.questions.find(q => q.id === d.questionId)?.coveredBy).map(d => ({ ...d,
        question: state.questions.find(q => q.id === d.questionId)?.text || '', status: state.questions.find(q => q.id === d.questionId)?.status || 'open' })),
      consultations: state.investigation.consultations,
    } : undefined,
    ...
    answerFetches: state.questions.filter(q => !q.coveredBy && (q.answer || q.reason)).map(q => ({ id: `answer:${q.id}`, questionId: q.id,
      status: q.status === 'answered' ? 'answered' : q.status === 'conflicting' ? 'conflicting' : q.status === 'partial' ? 'partial' : q.status==='unavailable'?'unavailable':q.inventory?'not-found':'not-yet-searched',
    ...
    causalBoard: { maturity: edges.length ? 'developing' : 'initial', focalNodeId: 'focal',
      ...
      branchRevisions: state.changes.filter(c => c.kind !== 'added').map(c => ({ previousTargetId: c.targetId, previousLabel: c.before||c.targetId,
        action: c.kind === 'retained' ? 'keep' : c.kind === 'rejected' ? 'reject' : 'revise', reason: c.reason, replacementIds: [c.targetId], evidenceIds: c.evidenceIds||[] })) },
    ...
    revision: { added: state.changes.filter(c => c.kind === 'added').map(c => c.targetId), changed: state.changes.filter(c => c.kind === 'revised').map(c => c.targetId),
      resolved: state.questions.filter(q=>q.status==='answered').map(q=>q.id), reopened: state.propositions.filter(p => p.humanStatus === 'reopened').map(p => p.id) },
    documentIds: input.documents.map(d => d.id),
    engineProgress: {assessmentGate:assessmentGate(state,execution), status: execution, phase: state.phase,pauseReason:state.pauseReason,model:{id:input.model.id,digest:input.model.digest,contextWindow:input.model.contextWindow,engineVersion:checkpointVersion(input,state)||'try5.1'},tasks: state.tasks.map(t => ({ id: t.id, kind: t.kind, owner: t.owner, status: t.status,traceId:t.traceId,failureCategory:t.failureCategory,
      ...
      sourceCoverage: state.sources.map(s => ({ id: s.id, label: s.label, spans: s.spans.length, limitations: s.limitations,
        readSpans:s.spans.filter(p=>state.investigation?.readSpanIds.includes(p.id)||state.tasks.some(t=>t.kind==='read'&&t.status==='completed'&&t.targetIds.includes(p.id))).length })) },
  };
}
