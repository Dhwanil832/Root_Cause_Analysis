...
export class D1EngineStore implements EngineStore {
  ...
  async claim(now: number, leaseMs: number) {
    ...
    const row = await this.db.prepare(`UPDATE engine_runs SET lease=?, lease_until=?, status='running'
      WHERE id = (SELECT r.id FROM engine_runs r WHERE r.status IN ('queued','running') AND r.lease_until < ?
      ...
      AND lease_until < ? RETURNING *`).bind(lease, now + leaseMs, now, now).first<RunRow>();
    if (!row) return null;
