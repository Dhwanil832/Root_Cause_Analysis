...
import { planContext,contextCapacity,CONTEXT_POLICY } from '../context/planner';
import { evidenceIndex } from '@/src/knowledge/retrieval/index';
import type { EngineState, EngineTask, EngineStore, TaskExecutorResult, TaskTrace, SourceSpan } from '../types';
...
import { investigationBase } from '../investigation/context';
import { isInvestigationKind } from '../investigation/contracts';
import {bundleSpans} from '@/src/knowledge/bundles';
...
const stageNames: Record<EngineTask['kind'], AgentId> = { read: 'evidence-reading', understand: 'incident-understanding', tag: 'tagging', specialist: 'other-novel',
  ...
  frame: 'causal-analysis', consult: 'other-novel', resolve: 'answer-fetching', refine: 'causal-analysis', connect: 'causal-analysis',respond:'other-novel',revise:'causal-analysis',role:'causal-verification' };
export function taskBase(state: EngineState, task: EngineTask) {
  if (isInvestigationKind(task.kind)) return investigationBase(state, task);
  ...
  if(task.kind==='role')base.target={...findingViews[0],proposedRole:state.propositions.find(p=>p.findingId===task.targetIds[0])?.type};
  ...
  if (task.kind === 'verify' || task.kind === 'actions') {
    ...
    base.target = 'label' in target ? { type: target.type, label: target.label, detail: target.detail }
      : { type: target.type, rationale: target.rationale, counterfactual: target.counterfactual, alternative: target.alternative, gap: target.gap,
        requiredJointConditions:target.jointConditions?.map(id=>state.findings.find(f=>f.id===id)?.statement),
...
export async function executeTask(state: EngineState, task: EngineTask, model: ModelDescriptor, trackId: string,
  ...
  // Include the exact citation/line grammar during selection, so removing the
  // old fractional reserve cannot let later grammar growth overflow the packet.
  const schemaForEvidence = (spans: SourceSpan[]) => taskSchema(task.kind,spans.map((_,i)=>`S${i+1}`),
    [...refs.findings.keys()],[...refs.questions.keys()],targetStatement,
    task.kind==='review' ? Object.fromEntries(spans.map((s,i)=>[`S${i+1}`,
      s.modality ? [] : s.text.split('\n').flatMap((line,n)=>line.trim()?[n+1]:[])])) : undefined,
    (base.boardNodes as Array<{ref:string}> | undefined)?.map(n => n.ref),base.assignedQuestions as string[]|undefined,
    Object.fromEntries([...refs.findings].map(([ref,id])=>[ref,state.findings.find(f=>f.id===id)!.revision])));
  const context = planContext({ model, system, schema:schemas[task.kind], base, index, query: task.query, required: task.kind==='interpret'?[]:bundleSpans(state,task),
    onlyRequired:task.kind==='read'||task.kind==='interpret',numberEvidenceLines:task.kind==='review',schemaForEvidence });
  ...
  const schema=context.schema, outputTokens=context.outputTokens;
  const negative = ['answer','review','causal','inquiry','verify','frame','consult','resolve','refine','connect','role','respond','revise'].includes(task.kind);
  ...
  const identity = { engine: ENGINE_VERSION, streamPolicy: model.provider === 'ollama' ? STREAM_REPETITION_POLICY : undefined,
    ...
    context: contextCapacity(model),contextPolicy:CONTEXT_POLICY,thinking,sampling:inference.sampling,
    inventory: negative ? state.sources.map(s => `${s.id}:${s.textHash}`).sort() : undefined };
  const key = await digest(identity);
  await controls?.trace?.request({...identity,outputTokens:model.provider==='ollama'?-1:null,
    referenceBindings:{sources:Object.fromEntries(refs.sources),findings:Object.fromEntries(refs.findings),questions:Object.fromEntries(refs.questions)},
    plannedAvailableOutputTokens:outputTokens ?? null,generationPolicy:'until-provider-completion-no-application-cap',
  ...
  const result = await runner({ stage: task.kind === 'specialist' || task.kind === 'consult' ? task.owner as AgentId : stageNames[task.kind], model,
    systemPrompt: system, schema, schemaName: `try7_${task.kind}`, evidencePacket: context.packet,
    contextWindow: contextCapacity(model),thinking,sampling:inference.sampling,signal:controls?.signal,onChunk:controls?.trace?.append,
