...
import {applyRole} from '../validation/role';
import {refreshIntegrity} from '../validation/integrity';
import {failureCategory} from '../validation/execution';
...
export async function workerStep(store: EngineStore, resolveModel: (id: string) => Promise<ModelDescriptor>, runner?: ProviderRunner,
  ...
  try {
    ...
    try {
      ...
      else if(task.kind==='role')applyRole(state,task,result.output,result.refs);
      ...
      delete task.failureCategory;
      if(state.quarantine.length>rejectedBefore) {
        const rejected=state.quarantine.slice(rejectedBefore);
        const usable = task.kind === 'connect' || (['frame', 'refine','revise','respond','resolve'].includes(task.kind) && hasUsablePosition(state));
      ...
      task.failureCategory=failureCategory(error);
      if(task.kind==='review'||task.kind==='interpret') {
        const finding=state.findings.find(f=>f.id===task.targetIds[0]);
