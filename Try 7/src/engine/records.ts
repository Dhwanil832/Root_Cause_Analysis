...
import { bindQuestion } from './questions/ledger';
...
export function ingestFinding(state: EngineState, task: EngineTask, row: z.infer<typeof findingSchema>, refs: References) {
  ...
  if (prior) {
    ...
    const scopeChanged=(['subject','predicate','location','time','unit','qualifiers'] as const).some(key=>prior[key]!==row[key]);
    if (prior.statement !== row.statement || newEvidence || scopeChanged) {
...
export function ingestQuestion(state: EngineState, task: EngineTask, row: z.infer<typeof questionSchema>, refs: References) {
  ...
  const prior = state.questions.find(q => normalize(q.text) === normalize(row.text) && normalize(q.subject) === normalize(row.subject)
    ...
    && normalize(q.decision) === normalize(row.decision) && normalize(q.intent) === normalize(row.intent) && !q.coveredBy);
  if (prior) { prior.owners = [...new Set([...prior.owners, task.owner])]; prior.spanIds = [...new Set([...prior.spanIds, ...spanIds])]; bindQuestion(prior,task); return prior.id; }
  ...
  bindQuestion(question,task);state.questions.push(question); return question.id;
}
