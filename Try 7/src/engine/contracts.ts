...
import { investigationSchemas, investigationSchema, isInvestigationKind } from './investigation/contracts';
...
export const schemas = { ...investigationSchemas, role:roleSchema, read: readSchema, understand: understandSchema, tag: tagSchema, specialist: readSchema,
...
export function taskSchema(kind: keyof typeof schemas, sources: string[], findings: string[], questions: string[], targetStatement = '', sourceLines?:Record<string,number[]>, nodeRefs?: string[], assignedQuestions?:string[], findingRevisions?:Record<string,number>): z.ZodType {
  if (isInvestigationKind(kind)) return investigationSchema(kind, findings, sources, questions, nodeRefs,assignedQuestions,findingRevisions);
  ...
  if (kind==='role') return roleSchema.extend({references:citations});
