...
import {admitDocuments,releaseHash} from '@/src/experiments/admission';
import {recoverReferenceQuestions} from '@/src/engine/questions/reference-recovery';
...
export async function enqueueRevision(db: D1Database, trackId: string, incident: string, model: ModelDescriptor,
  documents: DocumentRecord[], incoming: InvestigationAnswer[] = [], options: { releaseId?:string;expectedVersion?: number; storyRoundId?: string; humanDecision?: HumanDecision; initial?: boolean; reviewAfterReading?:boolean } = {}) {
  admitDocuments(documents,trackId);
  const releaseId=options.releaseId||options.storyRoundId||crypto.randomUUID();
  const payloadHash=await releaseHash(incident,documents,incoming,{expectedVersion:options.expectedVersion,model:model.id,digest:model.digest,humanDecision:options.humanDecision});
  const replay=async()=>{
    const saved=await db.prepare('SELECT * FROM evidence_releases WHERE track_id=? AND release_id=?').bind(trackId,releaseId).first<{payload_hash:string;number:number;run_id:string}>();
    if(!saved)return null;
    if(saved.payload_hash!==payloadHash)throw Error('Release ID was already used with a different payload.');
    const run=await db.prepare('SELECT status FROM engine_runs WHERE id=?').bind(saved.run_id).first<{status:string}>();
    return {version:saved.number,queued:!!run&&['queued','running'].includes(run.status),releaseId,replayed:true};
  };
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    admitDocuments(documents,trackId,number);
    input.release={id:releaseId,trackId,parentVersion:number-1,hash:payloadHash,
      sourceIds:['incident-description',...documents.map(d=>d.id),...input.answers.map(a=>a.sourceId||`answer:${a.questionId}`)]};
    ...
    try {
      await db.batch([db.prepare(`INSERT INTO engine_runs (id,track_id,number,parent_number,status,trigger,input_json,state_json,snapshot_json,created_at,updated_at)
        VALUES (?,?,?,?,'queued',?,?,NULL,?,?,?)`).bind(id, trackId, number, number - 1, trigger, await putJSON(db,input), await putJSON(db,snapshot), now, now),
        db.prepare('INSERT INTO evidence_releases (track_id,release_id,parent_number,payload_hash,run_id,number,created_at) VALUES (?,?,?,?,?,?,?)').bind(trackId,releaseId,number-1,payloadHash,id,number,now)]);
      return { version: number, queued: true,engineVersion:ENGINE_VERSION,reviewAfterReading:input.reviewAfterReading,releaseId,replayed:false };
    } catch (error) {
...
export async function retryFailedTasks(db: D1Database, trackId: string, options: ResumeOptions = {}) {
  ...
  if(paused) {
    ...
    if(state) {
      recoverReferenceQuestions(state);
      if (options.taskId) {
