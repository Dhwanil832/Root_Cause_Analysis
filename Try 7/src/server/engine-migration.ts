...
export async function migrateContinuousExecution(db:D1Database,runId:string,expectedGeneration:number,checkpointHash:string,reason:string) {
  if (!ENGINE_VERSION.startsWith('try5.')) throw Error('This engine requires a fresh model track. Import original evidence only; historical checkpoints cannot be migrated into the new investigation engine.');
