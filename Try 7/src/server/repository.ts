...
import { storeDocumentText, readDocumentText } from './document-text-storage';
...
export async function modelFromId(id: string): Promise<ModelDescriptor> {
  ...
  if (id.startsWith('ollama:')) {
    ...
    try {
      ...
      const metadata = await readOllamaMetadata(name, digest);
      capabilities = metadata.capabilities; contextWindow = metadata.contextWindow;
    } catch {
      // The provider stage will report availability if Ollama becomes unreachable.
    }
    return { id, name, provider: 'ollama', detail: 'Ollama · local', available: true, capabilities, digest, contextWindow };
  }
...
async function documentFromRow(row: DocumentRow): Promise<DocumentRecord> {
  return {
    id: row.id,
    scope: row.scope,
    title: row.title,
    ...
    extractedText: await readDocumentText(database(), row.extracted_text),
...
async function queryDocuments(sql: string, ...bindings: unknown[]) {
  ...
  return Promise.all(result.results.map(documentFromRow));
}
...
async function persistFile(
  ...
  const extracted = savedExtraction ? await readJSON<Awaited<ReturnType<typeof extractDocument>>>(database(), savedExtraction.result_json) : await extractDocument(bytes, file.name, file.type || 'application/octet-stream');
  if (!savedExtraction) await database().prepare('INSERT OR IGNORE INTO extraction_cache (cache_key,result_json,created_at) VALUES (?,?,?)').bind(cacheKey, await putJSON(database(), extracted), new Date().toISOString()).run();
  ...
  await database().prepare([
    ...
    record.revision, record.extractionStatus, record.extractionNotes, await storeDocumentText(database(), record.extractedText), record.createdAt,
  ).run();
  return record;
}
...
export async function getDocumentContent(id: string) {
  ...
  if (!row) return null;
  const document = await documentFromRow(row);
...
/** Preserve a document for one explicitly staged batch; never dispatch a revision here. */
export async function stageQuestionDocument(trackId: string, questionId: string, file: File, expectedVersion: number) {
  ...
  if (!track || current?.row.number !== expectedVersion || !['completed','partial'].includes(current.row.status))
    throw new Error('Document batch refers to a stale or unfinished version.');
  const digest = await sha256(await file.arrayBuffer());
  const existing = await queryDocuments('SELECT * FROM documents WHERE track_id=? AND question_id=? AND sha256=? AND introduced_version=?',
    trackId, questionId, digest, expectedVersion + 1);
  const document = existing[0] || await persistFile(file, 'question', {trackId, incidentId:track.incident_id, questionId, introducedVersion:expectedVersion+1});
  await attachDocument(trackId, document, expectedVersion+1);
  return document;
}
...
export async function addQuestionDocument(trackId: string, questionId: string, file: File, answer='') {
  ...
  const revision=await enqueueRevision(database(),trackId,incident.description,await modelFromId(track.model_id),await documentsForNextRelease(trackId),
...
export async function createIncident(description: string, modelIds: string[], starterFiles: File[] = [], options: { deferAnalysis?: boolean; title?: string; referenceFiles?: File[]; initialEvidenceFiles?: File[] } = {}) {
  ...
  for (const modelId of modelIds) {
    ...
    // Previously collected additional evidence can join the initial revision
    // without inventing an answer/question or creating a revision per file.
    for (const file of options.initialEvidenceFiles || []) {
      const document = await persistFile(file, 'question', { incidentId, trackId, introducedVersion: 1 });
      await attachDocument(trackId, document, 1);
    }
...
/** Association history retains withdrawn documents; admission does not. */
async function documentsForNextRelease(trackId:string) {
  const current=await latestInput(database(),trackId),all=await documentsForTrack(trackId);
  if(!current)return all;
  const currentIds=new Set(current.input.documents.map(d=>d.id));
  return all.filter(d=>currentIds.has(d.id)||d.introducedVersion===current.row.number+1);
}
...
export async function addTrackAnswers(trackId: string, incoming: InvestigationAnswer[], expectedVersion?: number, storyRoundId?: string, _strictCausal = true) {
  ...
  const result = await enqueueRevision(database(), trackId, incident.description, await modelFromId(track.model_id), await documentsForNextRelease(trackId), incoming, { expectedVersion, storyRoundId });
...
export async function addHumanReviewVersion(trackId: string, input: {
  ...
  const result = await enqueueRevision(database(), trackId, current.input.incident, current.input.model, current.input.documents, [], { humanDecision: decision,expectedVersion:current.row.number });
