...
export const openAiCompatibleProvider: ModelProvider = {
  ...
  async run<T>(request: StageRequest<T>): Promise<StageResponse<T>> {
    ...
    for (let attempt = 1; attempt <= (request.maxAttempts ?? 2); attempt++) {
      ...
      try {
        ...
        const response = await fetch(`${baseUrl.replace(/\/$/, '')}/${useResponses ? 'responses' : 'chat/completions'}`, {
          ...
          body: JSON.stringify(useResponses ? {
            ...
            store: false,
            ...(request.noTruncation ? { truncation: 'disabled' } : {}),
            ...(!request.noTruncation && request.outputTokens ? { max_output_tokens: request.outputTokens } : {}),
            ...
            text: { format: { type: 'json_schema', name: request.schemaName, strict: true, schema } },
              ...
              model: request.model.name,
              temperature: 0,
              ...(!request.noTruncation && request.outputTokens ? { max_completion_tokens: request.outputTokens } : {}),
              messages: [
          ...
          // No application deadline. Pause/lease cancellation still works.
