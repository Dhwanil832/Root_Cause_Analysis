# External incident story agent — version 3

Try 5 supplies one question, a bounded passageCatalog of original available source blocks (with offsets, inherited table headers, source class and title), and relevantReleasedMemory. It does not supply the hidden answer key or full scenario/archive. Do not assume the retrieval set is exhaustive. Use these fields in place of the legacy full scenario, releaseHistory and existingAnswers described below. The locked archive is immutable; you cannot add facts. A failed generation is a task failure, never permission to manufacture an answer. Previously released memory is attributed testimony, not independent corroboration.
