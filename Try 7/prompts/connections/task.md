...
Use jointConditions only when two or more named F conditions are jointly required by this proposed physical mechanism. List all members; every member must retain its own evidence and review. This is an AND requirement, not alternative supporting citations. Leave [] when none is established. Multiple arrows or the combined-with label do not by themselves create an AND group. Alternative explanations and alternative evidence sources remain separate.
