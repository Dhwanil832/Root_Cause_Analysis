...
export function fixtureRunner(log, { fail=()=>false, questionCount=1, noConsult=false, omitOldBranch=false }={}) {
  return async request => {
    ...
    const kind = request.schemaName.replace('try7_', ''), p = request.evidencePacket;
    let output;
    ...
    if(kind==='role')output={status:'valid',reason:'Scripted role assessment, not model reasoning.',protectiveFunction:'',references:[p.evidence[0].ref],decision};
    if(kind==='respond')output={responses:p.assignedQuestions.map(ref=>({question:ref,answerRevision:p.questions.find(q=>q.ref===ref).answerRevision,implication:'Fixture recipient consumed the recorded answer status.'})),directions:[],decision};
    if(kind==='revise')output={corrections:[],branches:[],impacts:p.impactTargets.map(t=>({target:t.target,action:'retain',reason:'Scripted retention for workflow testing.',findings:[]})),nodes:[],retiredNodes:[],withdrawnEdges:[],directions:[],summary:p.incidentContext.summary,decision};
