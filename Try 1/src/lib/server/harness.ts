...
function focalValue(output: Record<string, unknown>): string {
  const value = output.focalPoint ?? output.focal_point ?? output.focal ?? output.event ?? output.condition ?? output.statement;
  if (typeof value === "string" && value.trim()) return value.trim();

  // Some otherwise capable local models nest the requested field under a
  // response envelope. Accept those schema variants before falling back to an
  // evidence-backed incident statement instead of aborting the entire run.
  for (const key of ["result", "response", "data", "analysis"]) {
    const nested = output[key];
    if (nested && typeof nested === "object" && !Array.isArray(nested)) {
      const focal = focalValue(nested as Record<string, unknown>);
...
function focalPointFrom(output: Record<string, unknown>, input: CaseInput) {
  const focal = focalValue(output);
  if (focal) return focal;
  return input.facts[0]?.statement || input.incident;
}

function fallbackOutput(role: string): Record<string, unknown> {
  if (role.startsWith("5.1.1")) return { candidates: [] };
  if (role.startsWith("5.1.2")) return { stop: true, reason: "evidence_exhausted", explanation: "Model response unavailable; parked by harness fallback." };
  if (role.startsWith("5.2 Controllability")) return { actor: null, specificChange: null, explanation: "Model response unavailable." };
  if (role.startsWith("5.2 Control Gap")) return { safeguard: "Unknown", gap: "Model response unavailable.", nextCause: "Unknown" };
  if (role.startsWith("5.5.1")) return { causes: [] };
  if (role.startsWith("5.5.2")) return { checks: [] };
  if (role.startsWith("5.5.3")) return { sufficient: false, missing: ["Model response unavailable"], explanation: "Cannot establish sufficiency." };
  if (role.startsWith("5.6")) return { type: "standing_condition", area: "organization_management_system" };
  if (role.startsWith("5.7.1")) return { factIds: [] };
  if (role.startsWith("5.7.2")) return { supports: false, factIds: [], explanation: "Model response unavailable." };
  if (role.startsWith("5.7.3")) return { denies: false, factIds: [], explanation: "Model response unavailable." };
  if (role.startsWith("6.1")) return { canWritePreventiveFix: false, explanation: "Model response unavailable." };
  if (role.startsWith("6.2")) return { action: "", changedCondition: "" };
  if (role.startsWith("6.3")) return { preventsOrInterrupts: false, explanation: "Model response unavailable." };
  // For focal point and list construction, the existing code uses verified
  // incident facts and seed things when these fields are absent.
  return {};
}
...
export async function runHarness(input: CaseInput, provider: ProviderConfig, model: ModelConfig): Promise<RunResult> {
  ...
  const focalPoint = focalPointFrom(focal, input);
