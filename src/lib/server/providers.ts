...
export async function complete(provider: ProviderConfig, model: string, prompt: string, temperature = 0.2) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const body = await response.json() as { choices?: { message?: { content?: string | null; reasoning?: string; reasoning_content?: string } }[]; message?: { content?: string | null; thinking?: string } };
