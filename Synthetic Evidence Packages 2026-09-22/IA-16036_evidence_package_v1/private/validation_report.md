...
Date: 2026-09-22. Package status: **prepared_for_pilot**. Authored/self-reviewed by Codex. No independent technical reviewer, model run or semantic scorer is claimed.

## Executed checks

`package_tools.py` produced **247 passing structural checks, zero failing checks** for this package. Full check details are saved in [integrity_results.json](integrity_results.json). These are small integrity assertions, not 247 independent RCA evaluations.
...
| --- | ---: | ---: | ---: | ---: | ---: |
| B0 | 6 | 859 | 5424 | 6 | 859 |
| B1 | 3 | 531 | 3104 | 9 | 1390 |
| B2 | 4 | 589 | 3780 | 12 | 1872 |
...
- pass, self-review: fixed entity aliases, relative order, dates, state transitions and document-effective/issue dates cross-checked against the private scenario. The series bypass model is mechanically simple; both the normal-path closure and unavailable bypass are required. Pressure samples are not used to calculate a flow rate.
...
Permanent unknowns: crew intent, why the change impact field was No, why the draft remained unissued, complete damage assessment and quantitative air-flow behavior. Targets depending on missing delivery, contradictory assumptions or invalid technical details must be `not_testable`, not model failures.
