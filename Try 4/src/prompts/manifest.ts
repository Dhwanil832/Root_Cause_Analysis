...
import claimReview from '../../prompts/claim-review/system.md?raw';
import evidenceReading from '../../prompts/evidence-reading/system.md?raw';
...
import causalConsolidation from '../../prompts/causal-analysis/consolidation.md?raw';
import causalLinking from '../../prompts/causal-analysis/linking.md?raw';
import causalRevision from '../../prompts/causal-analysis/branch-revision.md?raw';
...
export const PROMPT_VERSION = 'try4.3.6-checkpoint-consolidation';

export function causalPhasePrompt(phase: 'consolidation' | 'linking' | 'branch-revision') {
  return `${shared}\n\n${({ consolidation: causalConsolidation, linking: causalLinking, 'branch-revision': causalRevision })[phase]}`;
}

// Preserve the input/configuration namespace of 4.3.0. Individual model-call
// keys additionally include the complete prompt, schema, packet and media.
// Change this epoch when that identity convention or provider semantics change.
export const CHECKPOINT_CONTEXT_VERSION = 'try4.3.0-resumable-evidence-review';

const stages: Partial<Record<AgentId, string>> = {
  'claim-review': claimReview,
  'evidence-reading': evidenceReading,
