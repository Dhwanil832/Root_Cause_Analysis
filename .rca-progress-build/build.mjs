...
function text(s,t,x,y,w,h,size=28,color='#233F45',bold=false){const a=s.shapes.add({geometry:'textbox',position:{left:x,top:y,width:w,height:h},fill:'none',line:{fill:'none',width:0}});a.text=t;a.text.style={typeface:font,fontSize:size,color,bold,autoFit:'none'};return a;}
function slide(title,notes){const s=p.slides.add();s.background.fill='#FAFBF8';text(s,title,64,48,1152,104,44,'#123E3A',true);text(s,String(p.slides.items.length).padStart(2,'0'),1150,662,65,28,18,'#60756E');s.speakerNotes.textFrame.setText(notes);return s;}
function row(s,y,label,body){text(s,label,64,y,330,64,28,'#166A57',true);text(s,body,424,y,790,105,28);}
...
text(s,'Progress since July 2026',64,228,1130,78,46,'#123E3A',true);
text(s,'An investigation prototype, recorded model experiments,\nand a staged evaluation framework',64,340,1120,110,32);
text(s,'Project #19  /  AI-Assisted Accident Investigation\nProgress through 22 September 2026',64,568,1070,65,22,'#60756E');
s=slide('Progress against the July next steps',`July baseline: ${sources.july}. Current evidence: ${sources.overview}; ${sources.try7}; ${sources.packages}. July already described a prototype and methodology. Do not imply the project began from zero after July.`);
...
s=slide('The prototype supports iterative investigation',`Sources: ${sources.overview}; ${sources.try7}. These are implemented workflow capabilities, not claims of consistently correct reasoning. A specialist is selected for relevant uncertainty. Application versions (Try 4–7) differ from investigation snapshots (V1–V3).`);
...
s=slide('Development runs exposed specific limitations',`Sources: ${sources.review}; ${sources.overview}. These experiments used changing inputs and configurations. They are development observations, not a controlled ranking of versions. Try 4 includes saved R3 versions and IA runs; Try 5 comparison snapshots were diagnostic; Try 6 published a partial IA V1/V2 pair.`);
...
s=slide('Recent changes address those failure modes',`Source: ${sources.try7}. The implementation receipt documents these changes and 97 deterministic checks (20 engine, 24 Try 7, 37 regression, 16 capacity), plus build/type checks. These are recorded checks, not checks rerun for this presentation. Full-plan acceptance remains incomplete.`);
...
text(s,'Synthetic reconstructions of development cases. Independent validation remains pending.',64,613,1110,38,21,'#8A4D23');
s=slide('Next milestone: a controlled V1–V3 investigation',`Sources: ${sources.plan}; ${sources.try7}; ${sources.packages}. Proposed next steps, not completed results or dated commitments. Begin with the instrument-air package prepared for pilot. Review R3 mechanics before scored use. Freeze the independent assessment protocol before held-out evaluation. Proposed partner input: technical review of scenario realism and causal interpretation.`);
...
for(let i=0;i<p.slides.items.length;i++){const blob=await p.export({slide:p.slides.items[i],format:'png',scale:1});await fs.writeFile(build+`/slide-${i+1}.png`,new Uint8Array(await blob.arrayBuffer()));}
await finalizePresentation({workspaceDir:root,candidatePath:build+'/candidate.pptx',finalPath:root+'/RCA Presentation Outputs/September Progress/RCA_Progress_Since_July_2026.pptx',pythonExecutable:'/Users/dhwanilchauhan/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3',integrityValidatorPath:skill+'/container_tools/inspect_presentation_package_integrity.py',layoutValidatorPath:skill+'/container_tools/inspect_presentation_layout_geometry.py',layoutArgs:['--expected-slide-size-emu','12192000,6858000','--validate-bullet-geometry','--validate-heading-fit'],fontPolicy:{basis:'design',families:[font]},verifyArtifactToolImport:true,receiptPath:build+'/validation.json'});
