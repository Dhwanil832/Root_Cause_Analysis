...
export const ENGINE_VERSION = 'try5.2.3';
export type TaskKind = 'read' | 'understand' | 'tag' | 'specialist' | 'broker' | 'answer' | 'interpret' | 'review' | 'causal' | 'inquiry' | 'verify' | 'actions';
...
export interface EngineQuestion {
  ...
  status: 'open' | 'answered' | 'partial' | 'conflicting' | 'awaiting-user' | 'covered';
  ...
  answerCompleteness?: 'answered' | 'partial' | 'conflicting' | 'not-found';
...
export interface EngineInput {
  /** Explicit operator-selected evidence update, not a full autonomous investigation. */
  controlledUpdate?: { policy: 'selected-answers-v1'; questionIds: string[]; reason: string };
...
export interface EngineState {
  controlledUpdate?: NonNullable<EngineInput['controlledUpdate']> & {
    stage: number; baselineRevisions: Record<string, number>; reviewFindingIds: string[];
  };
  /** Explicit operator-approved migration; immutable source inputs stay intact. */
  executionMigrations?: Array<{from:string;to:string;at:string;reason:string;checkpointHash:string}>;
  ...
  changes: Array<{ targetId: string; kind: 'added' | 'revised' | 'rechecked' | 'retained' | 'rejected'; reason: string }>;
