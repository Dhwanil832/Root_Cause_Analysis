...
export async function workerStep(store: EngineStore, resolveModel: (id: string) => Promise<ModelDescriptor>, runner?: ProviderRunner,
  ...
  try {
    ...
    if (!task) {
      ...
      run.status = state.pauseReason ? 'paused' : state.controlledUpdate || state.tasks.some(t => t.status === 'failed' || t.status === 'blocked') || state.quarantine.length ? 'partial' : 'completed';
      run.leaseUntil=0;
    ...
    try {
      ...
      if (['read','understand','tag','specialist','interpret','review'].includes(task.kind)) applyEvidenceTask(state, task, result.output, result.refs);
      ...
      if(state.quarantine.length>rejectedBefore) {
        ...
        task.status=onlyQuestionRejections(rejected)?'completed':'failed';
        task.error=`${rejected.length} ${task.status==='completed'?'optional question(s)':'invalid item(s)'} quarantined; valid sibling records were retained.`;
    ...
    if(task.status==='failed'&&['read','understand','tag'].includes(task.kind)) {
