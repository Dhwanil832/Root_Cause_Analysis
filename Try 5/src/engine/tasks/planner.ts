...
import { planControlledUpdate } from './controlled-update';
...
export async function initializeState(input: EngineInput, version: number, previous: EngineState | null): Promise<EngineState> {
  ...
  delete state.controlledUpdate;
  if (input.controlledUpdate) {
    if (!previous || !input.controlledUpdate.questionIds.length
      || new Set(input.controlledUpdate.questionIds).size !== input.controlledUpdate.questionIds.length
      || input.controlledUpdate.questionIds.some(id => !state.questions.some(q => q.id === id && !q.coveredBy)
        || !input.answers.some(a => a.questionId === id))) throw new Error('Controlled update requires a preserved parent and distinct answered parent questions.');
    state.controlledUpdate = { ...input.controlledUpdate, stage: 0,
      baselineRevisions: Object.fromEntries(previous.findings.map(f => [f.id, f.revision])), reviewFindingIds: [] };
    state.progressiveBoard = false;
    for (const target of [...state.propositions, ...state.relationships]) {
      target.status = 'unknown';
      target.reviewReason = 'New evidence update: independent board review pending. Previous judgment is preserved in V1.';
    }
  }
  ...
  for (const q of state.questions) { q.spanIds = q.spanIds.filter(id => valid.has(id)); q.answerSpanIds = q.answerSpanIds.filter(id => valid.has(id)); }
...
export async function planNext(state: EngineState) {
  ...
  if(state.controlledUpdate) return planControlledUpdate(state, add);
