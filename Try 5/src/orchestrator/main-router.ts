...
export async function routeAnalysis(_request: AnalysisRequest): Promise<AnalysisSnapshot> {
  void _request;
  throw new Error('Try 5 uses durable input revisions and the task worker. Submit through the incident/track repository; synchronous whole-investigation execution is disabled.');
}
