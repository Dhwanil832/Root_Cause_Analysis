...
export function applyQuestionTask(state: EngineState, task: EngineTask, raw: unknown, refs: References) {
  ...
  if (task.kind === 'broker') {
    ...
    question.coveredBy = target.id; question.status = 'covered'; question.reason = bindCommentary(out.reason,refs);
    target.owners = [...new Set([...target.owners, ...question.owners])];
    target.spanIds = [...new Set([...target.spanIds, ...question.spanIds])];
    if(question.reviewTargets?.length)target.reviewTargets=[...new Set([...(target.reviewTargets||[]),...question.reviewTargets])];
    if(question.inquiryTargets?.length)target.inquiryTargets=[...new Set([...(target.inquiryTargets||[]),...question.inquiryTargets])];
  } else {
...
export function reconcileAnswers(state: EngineState) {
  for (const q of state.questions.filter(q => !q.coveredBy)) {
    ...
    else if (claims.length && claims.every(f => f.status === 'supported') && q.answerSpanIds.length)
      q.status = q.answerCompleteness === 'answered' ? 'answered' : 'partial';
    else if (q.status === 'answered') q.status = 'partial';
