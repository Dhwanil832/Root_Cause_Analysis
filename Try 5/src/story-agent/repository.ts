import { database, addTrackAnswers } from '@/src/server/repository';
...
export async function generateStoryRound(trackId:string, questionIds?:string[]) {
  const scenario=await scenarioFor(trackId);
  ...
  const questions=existing ? JSON.parse(existing.questions_json) as typeof all : questionIds ? all.filter(q=>questionIds.includes(q.id)) : all;
