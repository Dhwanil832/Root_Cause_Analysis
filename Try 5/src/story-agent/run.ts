...
import { contextCapacity, plannedBytes, TaskContextError } from '@/src/engine/context/planner';
...
export function storyPacket(model: ModelDescriptor, scenario: StoryScenario, question: {id:string;text:string}, history: StoryOutput[]) {
  ...
  const limit = Math.max(0, (contextCapacity(model) - 3200 - 2048) * 2);
  ...
  // Reserve a portion for relevant past releases; neither archive is truncated in storage.
  for (const {p} of ranked) {
    packet.passageCatalog.push(p);
    if (plannedBytes(systemPrompt,packet,storySelectionSchema)>limit*0.8) packet.passageCatalog.pop();
  }
...
export async function answerStoryBatch(model:ModelDescriptor, scenario:StoryScenario,
  questions:{id:string;text:string}[], releaseHistory:StoryOutput[], _existingAnswers:unknown[]) {
  ...
  for (const question of questions) {
    ...
    const result=await runProviderStage({stage:'story-answering',model,systemPrompt,schema:storySelectionSchema,
      schemaName:'story_answer_selection',evidencePacket:packet,outputTokens:3200,contextWindow:contextCapacity(model),noTruncation:true,maxAttempts:1});
