...
export async function storyWorkerStep(db:D1Database, resolveModel:(id:string)=>Promise<ModelDescriptor>, answer=answerStoryBatch) {
  ...
  const job=await db.prepare(`UPDATE story_jobs SET status='running',lease=?,lease_until=?,attempts=attempts+1,updated_at=?
    WHERE id=(SELECT j.id FROM story_jobs j JOIN story_rounds r ON r.id=j.round_id
      ...
      ORDER BY j.created_at,j.id LIMIT 1) RETURNING *`).bind(lease,now+90000,new Date().toISOString(),now,now).first<Job>();
  if(!job) return {idle:true};
  let lost=false;
  const timer=setInterval(()=>{void db.prepare('UPDATE story_jobs SET lease_until=? WHERE id=? AND lease=? AND status=\'running\'')
    .bind(Date.now()+90000,job.id,lease).run().then(r=>{if(!r.meta.changes)lost=true;}).catch(()=>{lost=true;});},20000);
  try {
    ...
    const memory=[...history.results,...siblings.results].map(r=>JSON.parse(r.output_json).output as StoryOutput).filter(Boolean);
    ...
    const result=await answer(model,JSON.parse(row.scenario_json) as StoryScenario,[JSON.parse(job.question_json)],memory,[]);
