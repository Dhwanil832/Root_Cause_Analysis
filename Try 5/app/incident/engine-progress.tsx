...
export function EngineProgress({ progress, onRetry, onRefresh, onPause }: {
  ...
  return <section className="result-section" aria-live="polite">
    <div className="section-heading"><div><p className="eyebrow">Try 5 persistent engine</p><h2>Execution: {progress.status}</h2></div></div>
    <p>{done.length} / {progress.tasks.length} scheduled tasks completed · {done.filter(t => t.reused).length} reused · {failures.length} failed/blocked · {progress.quarantineCount} quarantined items.</p>
    <p>Execution completion is not a verified RCA. Evidence support and human approval are separate.</p>
    {progress.scopeNote && <p role="note"><strong>Run scope:</strong> {progress.scopeNote}</p>}
