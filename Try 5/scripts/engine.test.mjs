...
const model = { id: 'ollama:test-only', name: 'test-only', provider: 'ollama', digest: 'frozen-test-digest', available: true, detail: 'Deterministic test adapter, not a model evaluation' };
...
function fixtureRunner(log, fail = () => false) {
  return async request => {
    ...
    const p = request.evidencePacket, kind = request.schemaName.replace('try5_', ''), ref = p.evidence?.[0]?.ref;
    let output;
...
test('controlled revision preserves V1 and only processes selected answers before board verification', async () => {
  const previous = await initializeState(input, 1, null);
  const refs = {sources:new Map([['S1',previous.sources[0].spans[0].id]]),findings:new Map(),questions:new Map()};
  ingestRows(previous,{id:'read-old',kind:'read',owner:'evidence-reading'},{findings:[row('Old observation')],questions:[]},refs);
  const finding = previous.findings[0];
  finding.status='supported';
  const qid = ingestQuestion(previous,{owner:'stored-energy'},{text:'Which restraint was installed?',intent:'restraint',decision:'Resolve support',subject:'carriage',location:'line',time:'incident',references:['S1']},refs);
  ingestQuestion(previous,{owner:'human'},{text:'Who supervised the task?',intent:'supervision',decision:'Resolve roles',subject:'supervisor',location:'line',time:'incident',references:[]},refs);
  previous.propositions.push({id:'proposition:'+finding.id,findingId:finding.id,type:'condition',label:finding.statement,detail:'',spanIds:finding.spanIds,status:'supported',reviewReason:'Old support',humanStatus:'unreviewed'});
  previous.relationships.push({id:'old-edge',from:'proposition:'+finding.id,to:'focal',type:'enabled',rationale:'Old inference',counterfactual:'Unknown',alternative:'Unknown',gap:'',spanIds:finding.spanIds,findingIds:[finding.id],proposedBy:[finding.id],status:'supported',reviewReason:'Old review'});
  previous.tasks.push({id:'unfinished-old',kind:'specialist',status:'queued'});
  const original = JSON.stringify(previous);
  const update = {...input,documents:[...docs,document('new-record','A new record describes the carriage restraint.')],
    answers:[{questionId:qid,text:'The new record reports a restraint.',answeredAt:'2026-09-21T00:00:00Z'}],
    controlledUpdate:{policy:'selected-answers-v1',questionIds:[qid],reason:'Selected evidence release'}};
  const state = await initializeState(update,2,previous);
  assert.equal(JSON.stringify(previous),original);
  assert.equal(state.tasks.length,0);
  assert.equal(state.findings.length,previous.findings.length);
  assert.equal(state.questions.length,previous.questions.length);
  assert.equal(state.propositions[0].status,'unknown');
  assert.equal(state.relationships[0].status,'unknown');
  const answer = await planNext(state);
  assert.equal(answer.kind,'answer'); assert.deepEqual(answer.targetIds,[qid]);
  const newSpans = state.sources.find(s=>s.id==='new-record').spans.map(s=>s.id);
  assert.ok(newSpans.every(id=>answer.requiredSpanIds.includes(id)));
  answer.status='completed';
  state.questions.find(q=>q.id===qid).findingIds=[finding.id];
  let next = await planNext(state);
  assert.equal(next.kind,'review'); assert.deepEqual(next.targetIds,[finding.id]); next.status='completed';
  next = await planNext(state); assert.equal(next.kind,'causal'); next.status='completed';
  // A model-generated follow-up must remain visible without restarting broad work.
  ingestQuestion(state,{owner:'causal-analysis'},{text:'When was the restraint tested?',intent:'test',decision:'Validate condition',subject:'restraint',location:'line',time:'incident',references:[]},refs);
  next = await planNext(state); assert.equal(next.kind,'verify'); assert.equal(next.targetIds[0],'proposition:'+finding.id); next.status='completed';
  next = await planNext(state); assert.equal(next.kind,'verify'); assert.equal(next.targetIds[0],'old-edge'); next.status='completed';
  assert.equal(await planNext(state),null);
  assert.equal(await planNext(state),null);
  assert.equal(state.phase,10);
  assert.deepEqual(state.tasks.map(t=>t.kind),['answer','review','causal','verify','verify']);
  assert.equal(state.questions.length,previous.questions.length+1);
  assert.equal(JSON.stringify(previous),original);
});

test('controlled revision rejects absent, duplicate or unanswered parent questions', async () => {
  const previous=await initializeState(input,1,null);
  for (const questionIds of [[],['missing'],['missing','missing']])
    await assert.rejects(()=>initializeState({...input,controlledUpdate:{policy:'selected-answers-v1',questionIds,reason:'Test'}},2,previous),/distinct answered parent questions/);
});

test('controlled target store never claims another incident and respects paused predecessors', async () => {
  const db=database();
  try {
    await enqueueRevision(db,'unrelated',incident,model,docs);
    await enqueueRevision(db,'selected',incident,model,docs);
    const parent=await db.prepare('SELECT id FROM engine_runs WHERE track_id=?').bind('selected').first();
    await db.prepare("UPDATE engine_runs SET status='paused' WHERE id=?").bind(parent.id).run();
    await enqueueRevision(db,'selected',incident,model,docs,[{questionId:'a',text:'New evidence',answeredAt:'now'}]);
    const child=await db.prepare('SELECT id FROM engine_runs WHERE track_id=? AND number=2').bind('selected').first();
    const store=new D1EngineStore(db,child.id);
    assert.equal(await store.claim(Date.now(),1000),null);
    await db.prepare("UPDATE engine_runs SET status='partial' WHERE id=?").bind(parent.id).run();
    const claimed=await store.claim(Date.now(),1000);
    assert.equal(claimed.id,child.id);
    const unrelated=await db.prepare('SELECT status FROM engine_runs WHERE track_id=?').bind('unrelated').first();
...
test('context is bounded with a growing archive and never drops required evidence', async () => {
  ...
  assert.ok(packet.bytes <= (32768-3200-2048)*2);
...
test('read task includes only its original target; output receives remaining context, not 3200',async()=>{
  ...
  assert.ok(log[0].outputTokens>3200);
});
...
test('repetitive provider output continues to completion with telemetry and no record-count stop',async()=>{
  ...
  try {
    const result=await ollamaProvider.run({stage:'equipment-tool',model,systemPrompt:'Test',schema:readSchema,schemaName:'test',evidencePacket:{},
      noTruncation:true,outputTokens:25000,maxAttempts:2,onChunk:async text=>captured.push(text)});
...
test('production Qwen review uses the declared profile without changing other stages or models',async()=>{
  const {state,finding}=await reviewFixture('The pump stopped at 09:10.');
  ...
  assert.equal(requests[1].thinking,false);assert.equal(requests[1].sampling,undefined);
