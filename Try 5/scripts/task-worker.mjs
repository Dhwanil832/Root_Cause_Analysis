...
const base = process.env.RCA_APP_URL || `http://127.0.0.1:${process.env.RCA_PORT || 3015}`;
...
console.log(`Try 5 task worker polling ${base}; one model task at a time.`);
let lastError = '';
// Node fetch has a default headers timeout that can abandon a still-running
// local inference. Provider/task timeouts, not a second HTTP timer, own this call.
function tick() {
...
while (!stopping) {
  try {
    // No whole-investigation timeout. The provider bounds each individual task;
    // server-side leases recover an interrupted request without resetting a run.
