...
import { readOllamaMetadata } from './ollama-metadata';
...
export const ollamaProvider: ModelProvider = {
  ...
  async run<T>(request: StageRequest<T>): Promise<StageResponse<T>> {
    ...
    const contextWindow = request.contextWindow ?? request.model.contextWindow
      ?? (await readOllamaMetadata(modelName, request.model.digest, request.signal)).contextWindow;
    if (!contextWindow) throw new Error('Ollama did not report this model’s context capacity. No arbitrary context default was applied.');
    ...
    for (let attempt = 0; attempt < (request.maxAttempts ?? 2); attempt += 1) {
      ...
      // No application idle or elapsed-time cutoff. Operator/lease cancellation
      // and the model's physical context boundaries remain effective.
