...
export function project(state: EngineState, input: EngineInput, execution: RunStatus): AnalysisSnapshot {
  ...
  const activeQuestion = (q: EngineState['questions'][number]) => !state.investigation || state.investigation.directions.some(d => d.questionId === q.id)
    || q.reviewTargets?.some(id => state.investigation!.selectedFindingIds.includes(id));
  const questions: InvestigationQuestion[] = state.questions.filter(q => !q.coveredBy && activeQuestion(q)).map(q => ({
