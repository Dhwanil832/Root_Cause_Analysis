...
export function taskPrompt(kind: TaskKind, owner: string, verificationTarget?:'node'|'relationship') {
  ...
  const role = kind==='verify'&&verificationTarget==='node'?verifyNode:{ read, understand, tag, specialist, broker, answer, review, causal, inquiry, verify, actions, frame, consult, resolve, connect, refine: `${frame}\n\n${refine}` }[kind];
  ...
  return `${shared}\n\n${role}${kind === 'specialist' || kind === 'consult' ? `\n\nDomain mission:\n${mission}\n\nDomain kickoff examples (not a checklist to exhaust):\n${kickoff}` : ''}${kind === 'read' ? '\n\nTry 6 source notebook: extract distinct, incident-relevant observations once. Keep requirement scope and documented uncertainty. Do not speculate or multiply paraphrases. Return questions as an empty array: the incident-wide map decides evidence directions after seeing all documents.' : ''}`;
}
