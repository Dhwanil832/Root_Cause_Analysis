...
export async function POST(request: Request) {
  try {
    ...
    let files: File[] = [];
    let referenceFiles: File[] | undefined;
    let initialEvidenceFiles: File[] = [];
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (form.has('referenceFiles')) referenceFiles = form.getAll('referenceFiles').filter((item): item is File => item instanceof File && item.size > 0);
      initialEvidenceFiles = form.getAll('initialEvidenceFiles').filter((item): item is File => item instanceof File && item.size > 0);
    } else {
    ...
    const id = await createIncident(description.trim(), [...new Set(modelIds)], files, { deferAnalysis, title, referenceFiles, initialEvidenceFiles });
