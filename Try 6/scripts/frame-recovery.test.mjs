...
test('audited frame recovery retains original work and publishes without rereading or changing rejected proposals into facts', async()=>{
  ...
  try {
    ...
    const recovered=await recoverPartialFrame(db,row.id,row.generation,hash,'Test authorization');
    assert.equal(recovered.completedReadsRetained,3);
    ...
    assert.ok(restored.tasks.find(t=>t.id===frame.id).partial);
    assert.ok(restored.executionMigrations[0].backup);
    const after=[];await drain(store,fixtureRunner(after));
    assert.equal(after[0].schemaName,'try6_connect');
    assert.ok(!after.some(r=>r.schemaName==='try6_read'||r.schemaName==='try6_frame'));
    const [v]=await engineVersions(db,'track');assert.equal(v.executionStatus,'partial');assert.equal(v.analysis.investigationPosition.ready,true);
...
test('new map grammar separates selection from arrows and unusable frames still pause',async()=>{
  ...
  try{
    ...
    await drain(store,async request=>{
      ...
      if(request.schemaName==='try6_frame'){
...
test('usable frame with a rejected individual node continues to partial publication',async()=>{
  ...
  try{
    ...
    await drain(store,async request=>{
      const result=await runner(request);
      if(request.schemaName==='try6_read'&&request.evidencePacket.evidence[0].text.includes('4.5'))
