...
async function modelFromId(id: string): Promise<ModelDescriptor> {
...
export async function createIncident(description: string, modelIds: string[], starterFiles: File[] = []) {
  ...
  for (const modelId of modelIds) {
    ...
    const documentMedia = await loadDocumentMedia(trackDocuments);
    const analysis = await routeAnalysis({ incident: description, model, documents: trackDocuments, documentMedia });
    await database().prepare(
      'INSERT INTO versions (id, track_id, number, trigger, analysis_json, created_at) VALUES (?, ?, ?, ?, ?, ?)',
    ).bind(
      crypto.randomUUID(), trackId, 1,
      starterDocuments.length ? `Initial account with ${starterDocuments.length} starter document(s)` : 'Initial incident description',
...
export async function addTrackVersion(
  ...
  const answers: InvestigationAnswer[] = [
    ...previous.answers.filter((answer) => answer.questionId !== questionId),
    {
      questionId,
      text: answerText,
      sourceId: file?.fileKey || `user-answer:${questionId}`,
  ...
  await database().prepare(
    'INSERT INTO versions (id, track_id, number, trigger, analysis_json, created_at) VALUES (?, ?, ?, ?, ?, ?)',
  ).bind(crypto.randomUUID(), trackId, number, `Evidence added for ${questionId}`, JSON.stringify(analysis), now).run();
