      ...
      <div className="dashboard-grid">
        ...
        <section className="investigation-main">
          ...
          <div className="report-shortcuts">
            <Link className="button ghost-button" href={`/incident/${incident.id}/report?trackId=${track.id}&versionId=${version.id}`}>Open final report</Link>
          </div>
          ...
          <section className="result-section">
            ...
            <div className="claim-grid">{analysis.evidenceClaims.slice(0, 18).map((claim) => <article key={claim.id}><div><span>{claim.kind.replaceAll('-', ' ')}</span><b className={`evidence-status ${claim.status}`}>{claim.status}</b></div><p>{claim.text}</p><small>{claim.sourceIds.length ? `Sources: ${claim.sourceIds.join(', ')}` : 'Source still required'}</small></article>)}</div>
          </section>

          <section className="result-section causal-section">
            <div className="result-heading">
              <div><p className="eyebrow">Causal board · {analysis.causalBoard.maturity}</p><h2>From verified event toward testable causes</h2></div>
            ...
            <div className="causal-links">
              ...
              {analysis.causalBoard.edges.map((edge) => {
                ...
                return <article key={edge.id}><strong>{from}</strong><span>{edge.type.replaceAll('-', ' ')} →</span><strong>{to}</strong><p>{edge.rationale}</p></article>;
              })}
            </div>
