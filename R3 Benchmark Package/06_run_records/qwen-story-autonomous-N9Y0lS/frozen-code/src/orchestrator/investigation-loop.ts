...
export async function runInvestigationCycle(input: InvestigationCycleInput): Promise<AnalysisSnapshot> {
  ...
  const fail = async (stage: TraceEntry['stage'], caught: unknown) => {
    ...
    await checkpoint(stage, 'failed', { error: error.message, providerAttempts: (caught as { attempts?: unknown })?.attempts });
  };
  ...
  try {
    ...
    trace.push(result.trace);
    await checkpoint('question-broker', 'completed', { questions, skippedQuestions });
  } catch (error) {
