...
export const scenarioSchema = z.object({
  ...
  hiddenTruth: z.string().min(20).max(16000),
  sources: z.array(z.object({
    id: z.string().min(1).max(100),
    title: z.string().min(1).max(200),
    sourceClass: z.string().min(1).max(200),
    text: z.string().min(1).max(16000),
    available: z.boolean(),
  })).min(1).max(60),
}).superRefine((s,ctx) => {
  if(new Set(s.sources.map(x=>x.id)).size !== s.sources.length) ctx.addIssue({code:'custom',message:'Source IDs must be unique.'});
  if(JSON.stringify(s).length > 70000) ctx.addIssue({code:'custom',message:'Scenario exceeds 70,000 characters. Use a focused scenario.'});
});
export type StoryScenario = z.infer<typeof scenarioSchema>;
// Runtime bounds avoid expanding large maxLength repetitions in Ollama's grammar.
const boundedText = (limit:number, required=false) => z.string().refine(
  text => text.length <= limit && (!required || text.trim().length > 0),
  `Text must ${required?'be nonempty and ':''}contain at most ${limit} characters.`,
);
export const storyOutputSchema = z.object({answers:z.array(z.object({
  questionId:z.string(),
  status:z.enum(['answered','partial','unknown','unavailable']),
  answer:boundedText(2500,true),
  citations:z.array(z.object({sourceId:z.string(),quote:boundedText(1500,true)})),
  limitation:boundedText(1500),
}))});
...
export const storySelectionSchema = z.object({ answers: z.array(z.object({
  ...
  answer: boundedText(2500, true), passageIds: z.array(z.string()), limitation: boundedText(1500),
})) });
