  ...
  if (id.startsWith('ollama:')) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        capabilities = payload.models?.find((model) => model.name === name)?.capabilities || [];
      }
    } catch {
      // The provider stage will report availability if Ollama becomes unreachable.
    }
    return { id, name, provider: 'ollama', detail: 'Ollama · local', available: true, capabilities };
  }
...
export async function createIncident(description: string, modelIds: string[], starterFiles: File[] = [], options: { deferAnalysis?: boolean; title?: string } = {}) {
  ...
  for (const modelId of modelIds) {
    ...
    if (!options.deferAnalysis) await initializeTrack(trackId, false);
  }
  return incidentId;
}

async function runForCommit(trackId: string, targetVersion: number, request: Omit<AnalysisRequest, 'onCheckpoint'>, strict: boolean) {
  // Retry the SAME model and SAME evidence, never invent a successful version.
  // Failed cycles remain in checkpoints but are not immutable causal versions.
  for (let attempt = 1; attempt <= (strict ? 2 : 1); attempt++) {
    const onCheckpoint = checkpointWriter(trackId, crypto.randomUUID(), targetVersion);
    const analysis = await routeAnalysis({ ...request, onCheckpoint });
    const problems = strict ? causalCommitProblems(analysis) : [];
    if (!problems.length) return analysis;
    await onCheckpoint({ stage: 'revision', status: 'failed', payload: {
      reason: 'Commit gate rejected incomplete cycle', attempt, problems, uncommittedAnalysis: analysis,
    } });
    if (attempt === 2) throw new Error(`No version committed after two same-model cycles: ${problems.join('; ')}`);
  }
  throw new Error('No analysis available to commit.');
}

/** Deferred creation gives the unattended driver durable IDs before inference. */
export async function initializeTrack(trackId: string, strictCausal = true) {
  const track = await database().prepare('SELECT * FROM model_tracks WHERE id = ?').bind(trackId).first<TrackRow>();
...
async function documentsForTrack(trackId: string) {
  return queryDocuments([
    ...
    'WHERE td.track_id = ? ORDER BY td.introduced_version, d.created_at',
  ].join(' '), trackId);
}
...
export async function addTrackAnswers(
  trackId: string,
