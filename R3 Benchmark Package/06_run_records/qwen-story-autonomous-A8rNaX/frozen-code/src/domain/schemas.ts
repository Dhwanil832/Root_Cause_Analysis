...
export function brokerSchemaFor(candidateKeys: string[], catalogKeys: string[]) {
  ...
  return brokerSchema.extend({
    decisions: z.array(brokerSchema.shape.decisions.element.extend({
      candidateKey: z.enum(candidateKeys as [string, ...string[]]),
      coveredByCandidateKey: z.enum(['', ...new Set([...catalogKeys, ...candidateKeys])] as [string, ...string[]]),
    })).length(candidateKeys.length),
  });
}
...
export const evidenceAdjudicationSchema = z.object({
  assessments: z.array(z.object({
    ...
    applicability: z.enum(['direct-r3', 'similar-equipment', 'general-guidance', 'unknown']),
