...
import { causalReferenceProblems } from './reference-contract';

export async function runCausalAnalysis(model: ModelDescriptor, packet: unknown) {
  const result = await runAgentStage({
    stage: 'causal-analysis', model, schema: causalAnalysisSchema, schemaName: 'causal_analysis', packet,
    validateOutput: causalReferenceProblems,
  });
