...
export async function POST(request:Request,context:Context) {
  try {
    ...
    const body=await request.json() as {action?:string;scenario?:unknown;roundId?:unknown;strictCausal?:boolean};
    if(body.action==='create') return NextResponse.json(await createStoryScenario(trackId,body.scenario),{status:201});
    if(body.action==='generate') return NextResponse.json(await generateStoryRound(trackId));
